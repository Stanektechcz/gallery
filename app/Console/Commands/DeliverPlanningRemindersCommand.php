    ...
    public function handle(): int
    {
        ...
        foreach ($reminders as $reminder) {
            ...
            try {
                ...
                if (! $recipient || ! $reminder->event) {
                    throw new \RuntimeException('Chybí příjemce nebo akce.');
                }
        ...
        foreach ($capsules as $capsule) {
            if (! DB::table('time_capsules')->where('id', $capsule->id)->where('status', 'sealed')->update(['status' => 'delivered', 'delivered_at' => now(), 'updated_at' => now()])) {
                continue;
            }
