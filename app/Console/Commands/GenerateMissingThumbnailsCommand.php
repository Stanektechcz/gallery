...
{
    protected $signature   = 'gallery:thumbnails {--force : Regenerate even if thumbnail exists} {--recover : Download from Drive if local file missing}';
    protected $description = 'Generate thumbnails for media items that are missing them. Use --recover to download originals from Drive.';

    public function handle(): int
    {
        $query = MediaItem::whereDoesntHave('variants', fn($q) => $q->where('type', 'thumbnail'));
        if ($this->option('force')) $query = MediaItem::query();

        $total = $query->count();
        $this->info("Found {$total} media items without thumbnails.");
        if ($total === 0) { $this->info('Nothing to do.'); return 0; }

        $bar  = $this->output->createProgressBar($total);
        $done = 0; $fail = 0; $recovered = 0;
        ...
        $query->with('variants')->each(function (MediaItem $media) use ($bar, &$done, &$fail, &$recovered) {
            ...
            // Try to recover from Drive if local file missing
    ...
    private function downloadFromDrive(MediaItem $media): ?string
    {
        try {
            ...
            Log::warning('Drive download failed in thumbnails command', ['id' => $media->id, 'error' => $e->getMessage()]);
            return null;
        }
    }
    ...
    private function makeThumbnail(MediaItem $media, string $sourcePath): void
    {
        ...
        if (extension_loaded('imagick')) {
            try {
                ...
                if (Storage::disk('public')->put($thumbRel, fopen($tmp, 'rb'), 'public')) {
                    @unlink($tmp);
                    $media->variants()->updateOrCreate(['type' => 'thumbnail'], ['disk'=>'public','path'=>$thumbRel,'mime_type'=>'image/jpeg','size_bytes'=>Storage::disk('public')->size($thumbRel),'width'=>$size,'height'=>$size]);
                    return;
                }
                @unlink($tmp);
            } catch (\Throwable $e) { Log::warning('Imagick failed', ['id'=>$media->id,'e'=>$e->getMessage()]); }
        }
        ...
        if ($orig) {
            $media->variants()->updateOrCreate(['type'=>'thumbnail'],['disk'=>$orig->disk,'path'=>$orig->path,'mime_type'=>$orig->mime_type,'size_bytes'=>$orig->size_bytes,'width'=>null,'height'=>null]);
        }
    }
}
