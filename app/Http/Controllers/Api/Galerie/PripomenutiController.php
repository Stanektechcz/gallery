...
class PripomenutiController extends Controller
{
    ...
    public function __invoke(Request $request, WebPushService $push): JsonResponse
    {
        ...
        // Tiché hodiny druhého: push se nepošle (viz WebPushService) a řekne se proč.
        if (app(NotificationPreferenceService::class)->isQuiet($druhy)) {
            $druhyJmeno = Str::before(trim((string) $druhy->name), ' ');

            return response()->json(['ok' => true, 'doruceno' => 0, 'zprava' => $druhyJmeno.' má tiché hodiny — do telefonu teď nic nepřijde'], 200);
        }
