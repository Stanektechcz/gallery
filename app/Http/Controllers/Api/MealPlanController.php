...
class MealPlanController extends Controller
{
    ...
    public function tripStore(Request $request, int $tripId): JsonResponse
    {
        ...
        $meal = DB::transaction(function () use ($request, $trip, $day, $recipe, $album, $data, $plannedFor) {
            ...
            $startTime = $plannedFor->format('H:i');
            $endTime = $plannedFor->copy()->addMinutes($duration)->format('H:i');
    ...
    public function eventShopping(Request $request, string $uuid, string $key): JsonResponse
    {
        $this->write($request);
        $event = $this->event($request, $uuid);
        $item = $this->mealPlans->itemForEvent($event, $request->user(), $key);
    ...
    public function tripShopping(Request $request, int $tripId, string $key): JsonResponse
    {
        $this->write($request);
        $trip = $this->trip($request, $tripId);
        $item = $this->mealPlans->itemForTrip($trip, $request->user(), $key);
        abort_unless($item, 404);
    ...
    public function destroy(Request $request, string $uuid): JsonResponse
    {
        ...
        DB::transaction(function () use ($meal, $event, $generatedEvent) {
            if ($meal->trip_activity_id) {
                DB::table('trip_activities')->where('id', $meal->trip_activity_id)->delete();
            }
            if ($meal->cooking_session_id) {
                RecipeCookingSession::whereKey($meal->cooking_session_id)->where('status', '!=', 'completed')->delete();
            }
            if ($generatedEvent) {
                $event->delete();
            }
    ...
    private function reconcileShopping(int $spaceId, ?int $eventId, ?int $tripId, iterable $activeItems): void
    {
        ...
        foreach ($states as $state) {
            ...
            if (! $item) {
                if ($state->event_task_id) {
                    DB::table('event_tasks')->where('id', $state->event_task_id)->delete();
                }
                if ($state->packing_item_id) {
                    DB::table('trip_packing_items')->where('id', $state->packing_item_id)->delete();
                }
            ...
            if ($state->event_task_id) {
                DB::table('event_tasks')->where('id', $state->event_task_id)->update(['title' => $title, 'updated_at' => now()]);
            }
            if ($state->packing_item_id) {
                DB::table('trip_packing_items')->where('id', $state->packing_item_id)->update(['title' => $title, 'updated_at' => now()]);
            }
        }
    }
    ...
    private function saveShopping(Request $request, array $item, int $spaceId, ?CalendarEvent $event, ?object $trip): void
    {
        ...
        if (! empty($data['assigned_to'])) {
            abort_unless(DB::table('gallery_space_user')->where('gallery_space_id', $spaceId)->where('user_id', $data['assigned_to'])->exists(), 422, 'Položku lze přiřadit pouze členovi společného prostoru.');
        }
        ...
        $title = $this->shoppingTitle($item);
        $taskId = $existing?->event_task_id;
        $packingId = $existing?->packing_item_id;
        if ($event) {
            if (! $taskId) {
                $taskId = DB::table('event_tasks')->insertGetId(['event_id' => $event->id, 'assigned_to' => $assigned, 'title' => $title, 'notes' => $data['note'] ?? null, 'due_at' => $event->starts_at->copy()->subHour(), 'completed_at' => $checked ? now() : null, 'priority' => 'normal', 'sort_order' => ((int) DB::table('event_tasks')->where('event_id', $event->id)->max('sort_order')) + 1, 'created_at' => now(), 'updated_at' => now()]);
            } else {
                DB::table('event_tasks')->where('id', $taskId)->update(['assigned_to' => $assigned, 'title' => $title, 'notes' => $data['note'] ?? $existing?->note, 'completed_at' => $checked ? ($existing?->is_checked ? DB::table('event_tasks')->where('id', $taskId)->value('completed_at') ?? now() : now()) : null, 'updated_at' => now()]);
            }
        }
        if ($trip) {
            if (! $packingId) {
                $packingId = DB::table('trip_packing_items')->insertGetId(['uuid' => (string) Str::uuid(), 'trip_id' => $trip->id, 'created_by' => $request->user()->id, 'assigned_to' => $assigned, 'title' => $title, 'category' => 'food', 'quantity' => 1, 'is_essential' => false, 'is_packed' => $checked, 'packed_at' => $checked ? now() : null, 'packed_by' => $checked ? $request->user()->id : null, 'source_template' => 'meal_plan', 'sort_order' => ((int) DB::table('trip_packing_items')->where('trip_id', $trip->id)->max('sort_order')) + 1, 'created_at' => now(), 'updated_at' => now()]);
            } else {
                DB::table('trip_packing_items')->where('id', $packingId)->update(['assigned_to' => $assigned, 'title' => $title, 'is_packed' => $checked, 'packed_at' => $checked ? ($existing?->is_checked ? DB::table('trip_packing_items')->where('id', $packingId)->value('packed_at') ?? now() : now()) : null, 'packed_by' => $checked ? $request->user()->id : null, 'updated_at' => now()]);
            }
        }
    ...
    {
        return collect($this->recipes->payload($recipe, $servings))->only(['title', 'selected_servings', 'scale_factor', 'ingredients', 'steps', 'prep_minutes', 'cook_minutes', 'rest_minutes'])->all();
    }

    private function recipe(User $user, int $spaceId, string $uuid): Recipe
    {
        return Recipe::where('uuid', $uuid)->where('gallery_space_id', $spaceId)->where(fn ($query) => $query->where('status', 'published')->orWhere('created_by', $user->id))->firstOrFail();
    }

    private function event(Request $request, string $uuid): CalendarEvent
    {
        $this->available();

        return CalendarEvent::where('uuid', $uuid)->whereIn('gallery_space_id', $this->spaceIds($request->user()))->where(fn ($query) => $query->where('is_private', false)->orWhere('created_by', $request->user()->id)->orWhereHas('participants', fn ($participants) => $participants->whereKey($request->user()->id)))->firstOrFail();
    }
    ...
    private function trip(Request $request, int $id): object
    {
        ...
        return DB::table('trips')->where('id', $id)->whereIn('gallery_space_id', $this->spaceIds($request->user()))->firstOrFail();
    }
    ...
    {
        abort_if($request->user()->read_only_mode, 403, 'V režimu pouze pro čtení nelze jídelní plán měnit.');
    }

    private function available(): void
    {
        abort_unless(Schema::hasTable('planned_meals') && Schema::hasTable('meal_shopping_states'), 503, 'Pro propojené jídelní plánování dokončete databázové migrace.');
    }
}
