...
class RecipeCookingController extends Controller
{
    ...
    public function complete(Request $request, string $uuid, string $sessionUuid): JsonResponse
    {
        ...
        $finished = now();
        $started = $session->started_at ?? $finished;
        ...
        if ($media->isNotEmpty()) {
            $this->recipes->attachMedia($recipe, $request->user(), $media, $session, 'result');
        }
    ...
    private function finishGeneratedEvent(RecipeCookingSession $session, string $status, ?int $albumId = null): void
    {
        ...
        if (! $event || ! in_array($event->metadata['kind'] ?? null, ['recipe_cooking', 'trip_recipe_meal'], true)) {
            return;
        }
    ...
    {
        return Recipe::where('uuid', $uuid)->whereIn('gallery_space_id', $request->user()->gallerySpaces()->pluck('gallery_spaces.id'))->where(fn ($query) => $query->where('status', 'published')->orWhere('created_by', $request->user()->id))->firstOrFail();
    }

    private function write(Request $request): void
    {
        abort_if($request->user()->read_only_mode, 403, 'V režimu pouze pro čtení nelze vaření měnit.');
    }
}
