...
class TripTravelController extends Controller
{
    ...
    public function storeTransport(Request $request, int $tripId, TripPreparationTimelineService $preparation): JsonResponse
    {
        ...
        if ($selected) {
            DB::table('trip_travel_choices')->where('trip_id', $tripId)->where('kind', 'transport')->update(['is_selected' => false, 'updated_at' => now()]);
    ...
    public function storeAccommodation(Request $request, int $tripId, TripPreparationTimelineService $preparation): JsonResponse
    {
        ...
        $selected = $data['is_selected'] ?? true;
        if ($selected) {
            DB::table('trip_travel_choices')->where('trip_id', $tripId)->where('kind', 'accommodation')->update(['is_selected' => false, 'updated_at' => now()]);
        }
    ...
    private function secureUrl(?string $url): void
    {
        if ($url && ! Str::startsWith($url, 'https://')) {
            abort(422, 'Odkaz musí používat HTTPS.');
        }
    }
}
