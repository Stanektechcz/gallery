...
class EntertainmentController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        if (! empty($data['status']) && $data['status'] !== 'all') $query->where('status', $data['status']);
        if (! empty($data['type'])) $query->where('media_type', $data['type']);
        if (! empty($data['search'])) $query->where(fn ($search) => $search->where('title', 'like', '%' . $data['search'] . '%')->orWhere('original_title', 'like', '%' . $data['search'] . '%'));
        ...
        $proposals = DB::table('viewing_date_proposals as proposal')->leftJoin('cinema_showings as showing', 'showing.id', '=', 'proposal.cinema_showing_id')
            ...
            ->orderBy('proposal.starts_at')->get(['proposal.*', 'showing.uuid as showing_uuid', 'showing.booking_url']);
        ...
        return response()->json([
            ...
            'titles' => $titles->map(fn (EntertainmentTitle $title) => $this->titlePayload($title, $proposals, $proposalVotes, $progress, $sessions, $request->user()->id)),
            'cinema' => [
    ...
    public function search(Request $request): JsonResponse
    {
        ...
        try { $remote = $this->metadata->search($data['query'], $data['type'] ?? 'multi'); }
        catch (\Throwable $exception) { report($exception); $remote = []; }
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        if (($data['external_source'] ?? 'manual') === 'tmdb' && ! empty($data['external_id'])) {
            try { $data = array_merge($data, $this->metadata->details($data['media_type'] === 'series' ? 'tv' : 'movie', (int) $data['external_id'])); }
        ...
        $title->added_by ??= $request->user()->id; $title->gallery_space_id = $space->id; $title->save();
        return response()->json($this->basicTitle($title->fresh()), $title->wasRecentlyCreated ? 201 : 200);
    }

    public function update(Request $request, string $uuid): JsonResponse
    {
        $this->write($request); $title = $this->title($request, $uuid);
        ...
        if (($data['status'] ?? null) === 'watching' && ! $title->started_at) $data['started_at'] = now();
        if (($data['status'] ?? null) === 'watched') $data['watched_at'] = now();
        $title->update($data); return response()->json($this->basicTitle($title->fresh()));
    }
    ...
    {
        $title = $this->title($request, $uuid); $space = GallerySpace::findOrFail($title->gallery_space_id);
        ...
        $home = collect(); $cursor = now('Europe/Prague')->addDay()->startOfDay();
        while ($home->count() < 8 && $cursor->lte(now('Europe/Prague')->addDays(35))) {
            ...
            if (! $busy && in_array($cursor->dayOfWeekIso, [3, 5, 6, 7], true)) $home->push(['starts_at' => $start->toIso8601String(), 'ends_at' => $end->toIso8601String(), 'venue' => 'home', 'label' => $start->translatedFormat('l j. n. · H:i')]);
            $cursor->addDay();
        }
    ...
    {
        $this->write($request); $proposal = $this->proposal($request, $proposalUuid);
    ...
    {
        $this->write($request); $proposal = $this->proposal($request, $proposalUuid);
        if ($proposal->calendar_event_id) return response()->json($this->eventPayload(CalendarEvent::findOrFail($proposal->calendar_event_id)));
        $title = EntertainmentTitle::findOrFail($proposal->entertainment_title_id); $showing = $proposal->cinema_showing_id ? DB::table('cinema_showings')->find($proposal->cinema_showing_id) : null;
        $event = DB::transaction(function () use ($request, $proposal, $title, $showing) {
            $start = Carbon::parse($proposal->starts_at); $duration = $title->runtime_minutes ?: ($title->media_type === 'series' ? 90 : 150);
            $event = CalendarEvent::create([
                ...
                'metadata' => ['entertainment_uuid' => $title->uuid, 'proposal_uuid' => $proposal->uuid, 'booking_url' => $showing?->booking_url, 'source' => 'entertainment_planner'],
            ]);
            ...
            foreach ($members as $memberId) {
                ...
                foreach ([1440, 120] as $minutes) if ($start->copy()->subMinutes($minutes)->isFuture()) DB::table('event_reminders')->insert(['event_id' => $event->id, 'user_id' => $memberId, 'channel' => 'database', 'remind_at' => $start->copy()->subMinutes($minutes), 'status' => 'pending', 'created_at' => now(), 'updated_at' => now()]);
            }
            ...
            $title->update(['status' => 'scheduled']); return $event;
        });
    ...
    {
        $this->write($request); $title = $this->title($request, $uuid);
        $data = $request->validate(['watched_at' => 'nullable|date|before_or_equal:now', 'venue' => 'nullable|in:home,cinema,other', 'season_number' => 'nullable|integer|between:1,999', 'episode_from' => 'nullable|integer|between:1,9999', 'episode_to' => 'nullable|integer|between:1,9999', 'note' => 'nullable|string|max:5000', 'rating' => 'nullable|numeric|between:0.5,5', 'review' => 'nullable|string|max:5000', 'favorite_moment' => 'nullable|string|max:500', 'watch_again' => 'nullable|boolean']);
        ...
        if (! empty($data['rating'])) DB::table('entertainment_reviews')->updateOrInsert(['entertainment_title_id' => $title->id, 'viewing_session_id' => $sessionId, 'user_id' => $request->user()->id], ['rating' => $data['rating'], 'review' => $data['review'] ?? null, 'favorite_moment' => $data['favorite_moment'] ?? null, 'watch_again' => $data['watch_again'] ?? false, 'created_at' => now(), 'updated_at' => now()]);
        if ($title->media_type === 'series' && ! empty($data['season_number'])) DB::table('entertainment_progress')->updateOrInsert(['entertainment_title_id' => $title->id, 'user_id' => $request->user()->id], ['season_number' => $data['season_number'], 'episode_number' => $data['episode_to'] ?? $data['episode_from'] ?? 0, 'created_at' => now(), 'updated_at' => now()]);
    ...
    {
        $this->write($request); $data = $request->validate(['days' => 'nullable|integer|between:1,14']);
        try { return response()->json($this->cinema->sync(now('Europe/Prague')->startOfDay(), (int) ($data['days'] ?? 7))); }
        catch (\Throwable $exception) { report($exception); return response()->json(['message' => 'Program kina se nyní nepodařilo obnovit. Poslední uložený program zůstává dostupný.'], 502); }
    }

    public function importShowing(Request $request, string $showingUuid): JsonResponse
    {
        $this->write($request); $data = $request->validate(['gallery_space_id' => 'required|integer', 'propose' => 'nullable|boolean']); $space = $this->space($request, (int) $data['gallery_space_id']);
        ...
        if (! $title || $title->gallery_space_id !== $space->id) $title = EntertainmentTitle::firstOrCreate(['gallery_space_id' => $space->id, 'external_source' => 'cinema_city', 'external_id' => $showing->external_film_id ?: Str::slug($showing->title)], ['added_by' => $request->user()->id, 'media_type' => 'movie', 'title' => $showing->title, 'release_year' => $showing->release_year, 'runtime_minutes' => $showing->runtime_minutes, 'poster_url' => $showing->poster_url, 'status' => 'proposed']);
        DB::table('cinema_showings')->where('id', $showing->id)->update(['entertainment_title_id' => $title->id, 'updated_at' => now()]);
        if ($data['propose'] ?? true) $this->createShowingProposal($title, $showing, $request->user()->id);
    ...
    private function titlePayload(EntertainmentTitle $title, $proposals, $proposalVotes, $progress, $sessions, int $viewerId): array
    {
        ...
        return $this->basicTitle($title) + [
            ...
            'proposals' => $proposals->where('entertainment_title_id', $title->id)->map(function ($proposal) use ($proposalVotes, $viewerId) { $votes = collect($proposalVotes->get($proposal->id, [])); return ['uuid' => $proposal->uuid, 'starts_at' => Carbon::parse($proposal->starts_at)->toIso8601String(), 'venue' => $proposal->venue, 'place_name' => $proposal->place_name, 'note' => $proposal->note, 'status' => $proposal->status, 'showing_uuid' => $proposal->showing_uuid, 'booking_url' => $proposal->booking_url, 'votes' => $votes->countBy('response'), 'my_response' => $votes->firstWhere('user_id', $viewerId)?->response, 'event_uuid' => $proposal->calendar_event_id ? CalendarEvent::whereKey($proposal->calendar_event_id)->value('uuid') : null]; })->values(),
            'progress' => collect($progress->get($title->id, []))->values(), 'sessions' => collect($sessions->get($title->id, []))->take(5)->values(),
        ];
    }

    private function basicTitle(EntertainmentTitle $title): array { return collect($title->toArray())->except(['id', 'gallery_space_id', 'added_by', 'album_id'])->all(); }
    private function showings() { return DB::table('cinema_showings')->where('cinema_code', CinemaCityProgramService::CINEMA_CODE)->where('starts_at', '>', now())->orderBy('starts_at')->limit(80)->get()->map(fn ($item) => $this->showingPayload($item)); }
    private function showingPayload(object $item): array { return ['uuid' => $item->uuid, 'title' => $item->title, 'starts_at' => Carbon::parse($item->starts_at)->toIso8601String(), 'cinema_name' => $item->cinema_name, 'poster_url' => $item->poster_url, 'runtime_minutes' => $item->runtime_minutes, 'format' => $item->format, 'dubbed_language' => $item->dubbed_language, 'subtitles_language' => $item->subtitles_language, 'sold_out' => (bool) $item->sold_out, 'availability_ratio' => $item->availability_ratio, 'booking_url' => $item->booking_url, 'source_url' => $item->source_url]; }
    private function eventPayload(CalendarEvent $event): array { return ['uuid' => $event->uuid, 'title' => $event->title, 'starts_at' => $event->starts_at, 'href' => '/calendar/events/' . $event->uuid]; }
    ...
    private function title(Request $request, string $uuid): EntertainmentTitle { return EntertainmentTitle::where('uuid', $uuid)->whereIn('gallery_space_id', $request->user()->gallerySpaces()->pluck('gallery_spaces.id'))->firstOrFail(); }
    private function proposal(Request $request, string $uuid): object { $proposal = DB::table('viewing_date_proposals as proposal')->join('entertainment_titles as title', 'title.id', '=', 'proposal.entertainment_title_id')->where('proposal.uuid', $uuid)->whereIn('title.gallery_space_id', $request->user()->gallerySpaces()->pluck('gallery_spaces.id'))->select('proposal.*')->first(); abort_unless($proposal, 404); return $proposal; }
    private function write(Request $request): void { abort_if($request->user()->read_only_mode, 403, 'V režimu pouze pro čtení nelze filmový plán měnit.'); }
    private function available(): void { abort_unless(Schema::hasTable('entertainment_titles') && Schema::hasTable('viewing_date_proposals'), 503, 'Pro filmový plán dokončete databázové migrace.'); }
}
