...
class EntertainmentController extends Controller
{
    ...
    public function recordSession(Request $request, string $uuid): JsonResponse
    {
        ...
        if (! empty($data['rating'])) {
            DB::table('entertainment_reviews')->updateOrInsert(['entertainment_title_id' => $title->id, 'viewing_session_id' => $sessionId, 'user_id' => $request->user()->id], ['rating' => $data['rating'], 'review' => $data['review'] ?? null, 'favorite_moment' => $data['favorite_moment'] ?? null, 'watch_again' => $data['watch_again'] ?? false, 'created_at' => now(), 'updated_at' => now()]);
        }
    ...
    {
        return DB::table('cinema_showings')->where('cinema_code', CinemaCityProgramService::CINEMA_CODE)->where('starts_at', '>', now())->orderBy('starts_at')->limit(80)->get()->map(fn ($item) => $this->showingPayload($item));
    }

    private function showingPayload(object $item): array
    {
        return ['uuid' => $item->uuid, 'title' => $item->title, 'starts_at' => Carbon::parse($item->starts_at)->toIso8601String(), 'cinema_name' => $item->cinema_name, 'poster_url' => $item->poster_url, 'runtime_minutes' => $item->runtime_minutes, 'format' => $item->format, 'dubbed_language' => $item->dubbed_language, 'subtitles_language' => $item->subtitles_language, 'sold_out' => (bool) $item->sold_out, 'availability_ratio' => $item->availability_ratio, 'booking_url' => $item->booking_url, 'source_url' => $item->source_url];
    }
