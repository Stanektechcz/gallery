...
class CalendarAutomationController extends Controller
{
    ...
    public function storeGift(Request $request, GiftIdeaService $gifts): JsonResponse
    {
        ...
        if (! empty($data['source_url']) && ! Str::startsWith($data['source_url'], 'https://')) {
            abort(422, 'Odkazy na dárky musí používat HTTPS.');
        }
        if (! empty($data['person_id'])) {
            DB::table('people')->where('id', $data['person_id'])->where('gallery_space_id', $data['gallery_space_id'])->firstOrFail();
        }
        if (! empty($data['assigned_to'])) {
            abort_unless(DB::table('gallery_space_user')->where('gallery_space_id', $data['gallery_space_id'])->where('user_id', $data['assigned_to'])->exists(), 422, 'Dárek lze přiřadit pouze členovi společného prostoru.');
        }
    ...
    public function updateGift(Request $request, string $uuid, GiftIdeaService $gifts): JsonResponse
    {
        ...
        if (array_key_exists('assigned_to', $data) && $data['assigned_to'] !== null) {
            abort_unless(DB::table('gallery_space_user')->where('gallery_space_id', $gift->gallery_space_id)->where('user_id', $data['assigned_to'])->exists(), 422, 'Dárek lze přiřadit pouze členovi společného prostoru.');
        }
        if (! Schema::hasColumn('gift_ideas', 'assigned_to')) {
    ...
    public function updateDayNote(Request $request): JsonResponse
    {
        ...
        if (blank($data['content'] ?? null)) {
            ...
            return response()->json(['date' => $date, 'content' => '']);
        }
