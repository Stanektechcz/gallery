...
class GiftBudgetController extends Controller
{
    ...
    public function update(Request $request, string $uuid): JsonResponse
    {
        ...
        foreach (['title', 'occasion'] as $field) {
    ...
    private function payload(object $budget, Collection $gifts, int $userId): array
    {
        ...
        $matching = $gifts->filter(function (object $gift) use ($budget, $scope, $userId): bool {
            if (strtoupper($gift->currency ?? 'CZK') !== strtoupper($budget->currency)) {
                return false;
            }
            if (filled($budget->occasion) && strcasecmp((string) ($gift->occasion ?? ''), (string) $budget->occasion) !== 0) {
                return false;
            }
