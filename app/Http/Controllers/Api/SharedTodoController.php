...
class SharedTodoController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        if (! empty($data['search'])) {
            $query->where(fn ($search) => $search->where('title', 'like', '%'.$data['search'].'%')->orWhere('description', 'like', '%'.$data['search'].'%'));
        }
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        if ($clientRequestId && Schema::hasColumn('shared_todos', 'source_reference')) {
            ...
            if ($existing) {
                return response()->json($this->payload($existing->load(['assignee:id,name', 'creator:id,name', 'list:id,uuid,title,color,icon', 'children.assignee:id,name', 'comments.user:id,name'])));
            }
        }
        ...
        if ($createCalendar) {
            $this->todos->schedule($todo, $request->user(), ! empty($data['starts_at']) ? Carbon::parse($data['starts_at']) : null);
        }
    ...
    public function update(Request $request, string $uuid): JsonResponse
    {
        ...
        if (! empty($data['event_uuid'])) {
            $data['calendar_event_id'] = CalendarEvent::where('uuid', $data['event_uuid'])->where('gallery_space_id', $todo->gallery_space_id)->value('id');
        }
        if (array_key_exists('completed', $data)) {
            $next = $this->todos->complete($todo, $request->user(), (bool) $data['completed']);
            unset($data['completed']);
            if ($next) {
                $this->notifyAssignment($next, $request->user()->id, 'Opakovaný úkol je připraven');
            }
        }
        ...
        if (array_key_exists('assigned_to', $data) && (int) $previousAssignee !== (int) $data['assigned_to']) {
            $this->notifyAssignment($todo, $request->user()->id);
        }
    ...
    {
        $this->write($request);
        $data = $request->validate(['gallery_space_id' => 'required|integer', 'order' => 'required|array|max:300', 'order.*' => 'uuid']);
        $space = $this->space($request, (int) $data['gallery_space_id']);
        DB::transaction(function () use ($data, $space) {
            foreach ($data['order'] as $position => $uuid) {
                SharedTodo::where('uuid', $uuid)->where('gallery_space_id', $space->id)->update(['sort_order' => $position]);
            }
        });
    ...
    private function validateLinks(GallerySpace $space, array $data): void
    {
        if (! empty($data['assigned_to'])) {
            $this->member($space->id, (int) $data['assigned_to']);
        }
        if (! empty($data['trip_id'])) {
            DB::table('trips')->where('id', $data['trip_id'])->where('gallery_space_id', $space->id)->firstOrFail();
        }
        if (! empty($data['event_uuid'])) {
            CalendarEvent::where('uuid', $data['event_uuid'])->where('gallery_space_id', $space->id)->firstOrFail();
        }
    }
    ...
    private function syncDependencies(SharedTodo $todo, array $uuids): void
    {
        ...
        foreach ($ids as $id) {
            DB::table('shared_todo_dependencies')->insert(['todo_id' => $todo->id, 'depends_on_id' => $id, 'created_at' => now(), 'updated_at' => now()]);
        }
    }
    ...
    private function member(int $spaceId, int $userId): void
    {
        abort_unless(DB::table('gallery_space_user')->where('gallery_space_id', $spaceId)->where('user_id', $userId)->exists(), 422, 'Úkol lze přiřadit pouze členovi společného prostoru.');
    }
    ...
    private function todo(Request $request, string $uuid): SharedTodo
    {
        return SharedTodo::where('uuid', $uuid)->whereIn('gallery_space_id', $request->user()->gallerySpaces()->pluck('gallery_spaces.id'))->firstOrFail();
    }

    private function list(Request $request, string $uuid): SharedTodoList
    {
        return SharedTodoList::where('uuid', $uuid)->whereIn('gallery_space_id', $request->user()->gallerySpaces()->pluck('gallery_spaces.id'))->firstOrFail();
    }

    private function listInSpace(GallerySpace $space, string $uuid): SharedTodoList
    {
        return SharedTodoList::where('uuid', $uuid)->where('gallery_space_id', $space->id)->firstOrFail();
    }

    private function write(Request $request): void
    {
        abort_if($request->user()->read_only_mode, 403, 'V režimu pouze pro čtení nelze společné úkoly měnit.');
    }

    private function available(): void
    {
        abort_unless(Schema::hasTable('shared_todos') && Schema::hasTable('shared_todo_lists'), 503, 'Pro společné úkoly dokončete databázové migrace.');
    }
}
