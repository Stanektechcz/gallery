...
use App\Services\Planning\CalendarEventRevisionService;
...
class CalendarPlanningController extends Controller
{
    ...
    public function __construct(
        ...
        private readonly CalendarEventCreationService $eventCreation,
        private readonly CalendarEventLifecycleService $eventLifecycle,
        ...
        private readonly ReminderActionService $reminderActions,
        private readonly CalendarEventRevisionService $revisions,
    ) {}
    ...
    /** Export only events visible to the current member; private partner details never leak into the file. */
    public function exportIcs(Request $request)
    {
        ...
        $user = $request->user();
        $from = Carbon::parse($data['from'], 'Europe/Prague')->startOfDay();
        $to = Carbon::parse($data['to'], 'Europe/Prague')->endOfDay();
        abort_if($from->diffInDays($to) > 366, 422, 'Export kalendáře může mít nejvýše 367 dní.');
        if (! empty($data['gallery_space_id'])) {
            $this->ownedSpace($user, (int) $data['gallery_space_id']);
        }
        ...
        foreach ($events as $event) {
            ...
            $calendar[] = 'UID:'.$event->uuid.'@maki-gallery';
            $calendar[] = 'DTSTAMP:'.now('UTC')->format('Ymd\\THis\\Z');
            if ($event->all_day) {
                $start = $event->starts_at->copy()->setTimezone($timezone)->startOfDay();
                $end = ($event->ends_at ?: $event->starts_at)->copy()->setTimezone($timezone)->startOfDay()->addDay();
                $calendar[] = 'DTSTART;VALUE=DATE:'.$start->format('Ymd');
                $calendar[] = 'DTEND;VALUE=DATE:'.$end->format('Ymd');
            } else {
                $start = $event->starts_at->copy()->setTimezone($timezone);
                $end = ($event->ends_at ?: $event->starts_at->copy()->addHour())->copy()->setTimezone($timezone);
                $calendar[] = 'DTSTART;TZID='.$escape($timezone).':'.$start->format('Ymd\\THis');
                $calendar[] = 'DTEND;TZID='.$escape($timezone).':'.$end->format('Ymd\\THis');
            }
            $calendar[] = 'SUMMARY:'.$escape($event->title);
            if ($event->description) {
                $calendar[] = 'DESCRIPTION:'.$escape($event->description);
            }
            if ($event->place_name) {
                $calendar[] = 'LOCATION:'.$escape($event->place_name);
            }
            $calendar[] = 'STATUS:'.($event->status === 'confirmed' ? 'CONFIRMED' : 'TENTATIVE');
            $calendar[] = 'END:VEVENT';
        }
        $calendar[] = 'END:VCALENDAR';
        $name = 'maki-kalendar-'.$from->format('Y-m-d').'-'.$to->format('Y-m-d').'.ics';
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        if ($clientRequestId && Schema::hasColumn('calendar_events', 'source_reference')) {
            $existing = CalendarEvent::query()
                ->where('gallery_space_id', $space->id)
                ->where('created_by', $user->id)
                ->where('source_reference', $clientRequestId)
                ->first();
            if ($existing) {
                return response()->json($this->eventPayload($existing, $user));
            }
            $data['created_from'] = 'mobile';
            $data['source_reference'] = $clientRequestId;
        }
        $this->validateTripAndAlbum($data, $space->id);

        $event = $this->eventCreation->create($space, $user, $data, [$user->id]);
    ...
    public function history(Request $request, string $uuid): JsonResponse
    {
        ...
        return response()->json(['available' => $this->revisions->available(), 'revisions' => $this->revisions->history($event)->values()]);
    }

    public function restoreRevision(Request $request, string $uuid, string $revisionUuid): JsonResponse
    {
        $user = $request->user();
        $event = $this->findVisibleEvent($user, $uuid);
        $this->ensureCanEdit($event, $user);
        abort_unless($this->revisions->available(), 503, 'Pro historii úprav dokončete migrace aplikace.');
        $revision = $this->revisions->revision($event, $revisionUuid);
        abort_unless($revision, 404, 'Požadovaná verze akce nebyla nalezena.');
        $restore = $this->revisions->restoreData($revision);
        ...
        AuditLog::record('calendar.event.restore', $event, ['revision_uuid' => $revisionUuid, 'changed_fields' => $changed]);
        $this->notifyParticipants($event, $user, 'calendar.restored', "Akce byla vrácena k předchozí verzi: {$event->title}");
    ...
    public function update(Request $request, string $uuid): JsonResponse
    {
        ...
        $event = $this->findVisibleEvent($user, $uuid);
        $this->ensureCanEdit($event, $user);
        $data = $this->validatedEvent($request, true, $event);
        if (array_key_exists('metadata', $data)) {
            $data['metadata'] = array_merge(is_array($event->metadata) ? $event->metadata : [], $data['metadata'] ?? []);
        }
        if ($event->status === 'completed' && in_array($data['status'] ?? null, ['planned', 'confirmed'], true)) {
            $metadata = array_merge(is_array($event->metadata) ? $event->metadata : [], $data['metadata'] ?? []);
            unset($metadata['auto_completed_at'], $metadata['auto_completed_reason']);
        ...
        $changes = collect($this->revisions->snapshot($event))
            ...
            ->keys()->values()->all();
        if ($changes !== []) {
            $this->revisions->record($event, $user->id, 'update', $revisionSnapshot, $changes);
        }
        AuditLog::record('calendar.event.update', $event, ['changed_fields' => $changes]);
        $notificationType = $event->status === 'cancelled' ? 'calendar.cancelled' : 'calendar.updated';
        $notificationText = $event->status === 'cancelled' ? "Akce byla zrušena: {$event->title}" : "Akce byla upravena: {$event->title}";
        $this->notifyParticipants($event, $user, $notificationType, $notificationText);
    ...
    public function destroy(Request $request, string $uuid): JsonResponse
    {
        ...
        $title = $event->title;
        $this->notifyParticipants($event, $request->user(), 'calendar.deleted', "Akce byla odstraněna: {$title}");
        AuditLog::record('calendar.event.delete', $event, ['title' => $title, 'trip_id' => $event->trip_id, 'album_id' => $event->album_id]);
        $event->delete();
    ...
    public function updateTask(Request $request, string $uuid, int $taskId): JsonResponse
    {
        ...
        $task->update($data);
        if (array_key_exists('assigned_to', $data) && (int) $data['assigned_to'] !== (int) $previousAssignee) {
            $this->notifyTaskAssignee($event, $task, $request->user()->id);
        }
    ...
    public function storeAttachment(Request $request, string $uuid): JsonResponse
    {
        ...
        if (empty($data['media_item_id']) && empty($data['external_url']) && empty($data['reference_code'])) {
            abort(422, 'Přidejte médium, odkaz nebo referenci.');
        }
        if (! empty($data['external_url']) && ! Str::startsWith($data['external_url'], 'https://')) {
            abort(422, 'Odkazy musí používat HTTPS.');
        }
        if (! empty($data['media_item_id'])) {
            MediaItem::where('id', $data['media_item_id'])->where('gallery_space_id', $event->gallery_space_id)->whereNull('trashed_at')->firstOrFail();
        }
    ...
    public function applyMediaSuggestions(Request $request, string $uuid): JsonResponse
    {
        ...
        if ($ids->isEmpty() && $uuids->isEmpty()) {
            abort(422, 'Vyberte alespoň jednu fotografii nebo video.');
        }
        ...
        $media = MediaItem::query()
            ...
            ->where(function ($query) use ($ids, $uuids) {
                ...
                if ($uuids->isNotEmpty()) {
                    $query->{$ids->isNotEmpty() ? 'orWhereIn' : 'whereIn'}('uuid', $uuids);
                }
            })
            ->get();
    ...
    public function createSharedMemory(Request $request, string $uuid): JsonResponse
    {
        ...
        if (! $existing && $event->trip_id) {
            $existing = DB::table('shared_memory_moments')->where('trip_id', $event->trip_id)->first();
        }
        if ($existing) {
            DB::table('shared_memory_moments')->where('id', $existing->id)->update($row);
            $memoryId = $existing->id;
        } else {
            $memoryId = DB::table('shared_memory_moments')->insertGetId($row + ['uuid' => (string) Str::uuid(), 'created_at' => now()]);
        }
    ...
    public function updateReflection(Request $request, string $uuid): JsonResponse
    {
        ...
        if ($existing) {
            ...
            $id = DB::table('calendar_event_reflections')->insertGetId($row + ['uuid' => (string) Str::uuid(), 'calendar_event_id' => $event->id, 'created_by' => $request->user()->id, 'created_at' => now()]);
        }
    ...
    public function scheduleRevisit(Request $request, string $uuid): JsonResponse
    {
        ...
        if ($existing) {
            return response()->json($this->eventPayload($existing, $request->user()));
        }
        ...
        if (! $members) {
            $members = DB::table('gallery_space_user')->where('gallery_space_id', $source->gallery_space_id)->pluck('user_id')->all();
        }
    ...
    public function story(Request $request, string $uuid): JsonResponse
    {
        ...
        DB::transaction(function () use ($event, $media, $reflection, $partnerPerspectives, $summary, $request) {
            ...
            $album = $event->album_id
                ? Album::query()->whereKey($event->album_id)->where('gallery_space_id', $event->gallery_space_id)->first()
                : null;
            if (! $album || ! Schema::hasTable('album_story_blocks')) {
                return;
            }
            ...
            if ($photoUuids) {
                $blocks[] = ['type' => 'photo', 'content' => ['media_uuids' => $photoUuids, 'layout' => count($photoUuids) > 1 ? 'grid2' : 'single']];
            }
            $videoUuid = $media->firstWhere('media_type', 'video')?->uuid;
            if ($videoUuid) {
                $blocks[] = ['type' => 'video', 'content' => ['media_uuid' => $videoUuid]];
            }
            ...
            if ($baseOrder === null) {
                $baseOrder = ((int) ($existing->max('sort_order') ?? -1)) + 1;
            }
            $types = collect($blocks)->pluck('type')->all();
            foreach ($generated->whereNotIn('type', $types) as $block) {
                DB::table('album_story_blocks')->where('id', $block->id)->delete();
            }
    ...
    public function storeInbox(Request $request, TravelInboxService $travelInbox): JsonResponse
    {
        ...
        $space = $this->ownedSpace($user, (int) $data['gallery_space_id']);
        if (! empty($data['source_url']) && ! Str::startsWith($data['source_url'], 'https://')) {
            abort(422, 'Odkazy musí používat HTTPS.');
        }
        $data = $this->validateInboxLinks($data, $space->id);
        $item = $travelInbox->create($space->id, $user->id, $data, 'manual');
    ...
    public function timeCapsules(Request $request): JsonResponse
    {
        ...
        if (empty($data['event_uuid'])) {
            return response()->json($query->where('created_by', $user->id)->orderBy('deliver_at')->get());
        }
    ...
    public function storeTimeCapsule(Request $request): JsonResponse
    {
        ...
        $space = $this->ownedSpace($user, (int) $data['gallery_space_id']);
        if (! empty($data['recipient_user_id']) && ! $space->members()->whereKey($data['recipient_user_id'])->exists() && $space->owner_id !== (int) $data['recipient_user_id']) {
            abort(422, 'Příjemce není členem prostoru.');
        }
        if (! empty($data['event_id'])) {
        ...
        if (! empty($data['media_item_id'])) {
            MediaItem::where('id', $data['media_item_id'])->where('gallery_space_id', $space->id)->firstOrFail();
        }
    ...
    public function sharedSlots(Request $request): JsonResponse
    {
        ...
        for ($day = $from->copy(); $day->lt($until) && count($slots) < 8; $day->addDay()) {
            ...
            foreach ($members as $member) {
                ...
                if (! $memberRanges) {
                    $memberRanges = [['start' => $day->copy()->setTime(18, 0), 'end' => $day->copy()->setTime(21, 0)]];
                }
                $ranges = $ranges === null ? $memberRanges : $this->intersectTimeRanges($ranges, $memberRanges);
            }
            foreach ($ranges ?? [] as $range) {
                for ($start = $range['start']->copy(); $start->copy()->addMinutes($duration)->lte($range['end']) && count($slots) < 8; $start->addHour()) {
                    $end = $start->copy()->addMinutes($duration);
                    $busy = $events->contains(fn (CalendarEvent $event) => $event->starts_at->lt($end) && ($event->ends_at ?? $event->starts_at)->gt($start));
                    if (! $busy) {
                        $slots[] = ['starts_at' => $start->toIso8601String(), 'ends_at' => $end->toIso8601String(), 'duration_minutes' => $duration];
                    }
                }
            }
        }
    ...
    private function validatedEvent(Request $request, bool $partial, ?CalendarEvent $existing = null): array
    {
        ...
        if ($effectiveStart && $effectiveEnd && $effectiveEnd->lt($effectiveStart)) {
            abort(422, 'Konec akce nesmí být před jejím začátkem.');
        }
        if (isset($data['starts_at']) && isset($data['recurrence_rule']['until']) && Carbon::parse($data['recurrence_rule']['until'])->lt(Carbon::parse($data['starts_at'])->startOfDay())) {
            abort(422, 'Konec opakování nesmí být před začátkem akce.');
        }
    ...
    {
        $event->load(['creator:id,name', 'participants:id,name,email', 'tasks.assignee:id,name', 'attachments.media:id,uuid,display_title,original_filename', 'reminders.user:id,name']);
        $payload = $event->toArray();
        $payload['reminders'] = $event->reminders->map(function (EventReminder $reminder) use ($event, $viewer): array {
            $reminder->setRelation('event', $event);
            $canSeeHistory = $viewer && ((int) $reminder->user_id === (int) $viewer->id
                || ($reminder->user_id === null && (int) $event->created_by === (int) $viewer->id));
        ...
        $payload['can_edit'] = $viewer ? $this->canEdit($event, $viewer) : false;
        ...
        $payload['calendar_form_reminders'] = $event->reminders
            ...
            ->map(fn (EventReminder $reminder) => [
                ...
                'minutes_before' => max(0, (int) round(($event->starts_at->getTimestamp() - $reminder->remind_at->getTimestamp()) / 60)),
            ])->values();
        ...
        $payload['planning_items'] = Schema::hasTable('travel_inbox_items')
            ? DB::table('travel_inbox_items')->where('gallery_space_id', $event->gallery_space_id)
                ...
                ->where(function ($query) use ($event) {
                    ...
                    if ($event->trip_id) {
                        $query->orWhere('trip_id', $event->trip_id);
                    }
                })
     ...
     */
    private function syncExperienceAlbum(CalendarEvent $event, int $actorId, Collection $media): Album
    {
        return $this->experienceMedia->sync($event, $actorId, $media);
    }

    private function occurrences(CalendarEvent $event, Carbon $from, Carbon $to, ?User $viewer = null): array
    {
        $rule = $event->recurrence_rule;
        if (empty($rule['frequency'])) {
            $row = $event->toArray();
    ...
    private function canEdit(CalendarEvent $event, User $user): bool
    {
        ...
        if ($event->participants()->whereKey($user->id)->wherePivotIn('role', ['owner', 'editor'])->exists()) {
            return true;
        }
    ...
    {
        if ($userId && ! $event->participants()->whereKey($userId)->exists()) {
            abort(422, 'Úkol lze přiřadit pouze účastníkovi akce.');
        }
    }
    ...
    private function validateTripAndAlbum(array $data, int $spaceId): void
    {
        if (! empty($data['trip_id'])) {
            DB::table('trips')->where('id', $data['trip_id'])->where('gallery_space_id', $spaceId)->firstOrFail();
        }
        if (! empty($data['album_id'])) {
            DB::table('albums')->where('id', $data['album_id'])->where('gallery_space_id', $spaceId)->firstOrFail();
        }
    }
    ...
    private function validateInboxLinks(array $data, int $spaceId): array
    {
        ...
        if (! empty($data['event_id'])) {
            CalendarEvent::where('id', $data['event_id'])->where('gallery_space_id', $spaceId)->firstOrFail();
        }
        ...
        if ($day && ! empty($data['trip_id']) && (int) $data['trip_id'] !== (int) $day->trip_id) {
            abort(422, 'Den itineráře musí patřit ke zvolené cestě.');
        }
        if ($day && empty($data['trip_id'])) {
            $data['trip_id'] = $day->trip_id;
        }
        if (! empty($data['trip_activity_id'])) {
            ...
            if ($day && (int) $activity->trip_day_id !== (int) $data['trip_day_id']) {
                abort(422, 'Aktivita nepatří do vybraného dne.');
            }
            if (! empty($data['trip_id']) && (int) $activity->trip_id !== (int) $data['trip_id']) {
                abort(422, 'Aktivita nepatří ke zvolené cestě.');
            }
            if (empty($data['trip_id'])) {
                $data['trip_id'] = $activity->trip_id;
            }
        }
    ...
    private function syncParticipants(CalendarEvent $event, array $ids, User $actor): void
    {
        ...
        if (count(array_unique($ids)) !== count($valid)) {
            abort(422, 'Všichni účastníci musí být členy společného prostoru.');
        }
    ...
    private function replaceParticipants(CalendarEvent $event, array $ids, User $actor): void
    {
        ...
        $sync = $valid->mapWithKeys(function (int $id) use ($event, $existing) {
            $current = $existing->get($id)?->pivot;

            return [$id => [
                'role' => $id === (int) $event->created_by ? 'owner' : ($current?->role ?? 'guest'),
    ...
    private function reschedulePendingReminders(CalendarEvent $event, Carbon $originalStartsAt): void
    {
        $delta = $event->starts_at->getTimestamp() - $originalStartsAt->getTimestamp();
        if ($delta === 0) {
            return;
        }
        $event->reminders()->where('status', 'pending')->whereNull('snoozed_until')->get()->each(function (EventReminder $reminder) use ($delta): void {
            $reminder->remind_at = $reminder->remind_at->copy()->addSeconds($delta);
            if ($reminder->original_remind_at) {
                $reminder->original_remind_at = $reminder->original_remind_at->copy()->addSeconds($delta);
            }
            $reminder->save();
        });
    }

    private function comparableEventValue(mixed $value): mixed
    {
    ...
    private function syncReminders(CalendarEvent $event, array $reminders, User $actor): void
    {
        // Editing a calendar form must never erase reminders created by trips,
        // reservations, recipes or a partner's personal reminder.
        // Legacy reminders without provenance are deliberately preserved: they
        // may have originated in an older trip/recipe/reservation workflow.
        $event->reminders()->where('automation_source', 'calendar_form')->delete();
        foreach ($reminders as $reminder) {
            $recipient = $reminder['user_id'] ?? $actor->id;
            $this->ensureParticipant($event, $recipient);
            ...
            $event->reminders()->create([
                ...
                'status' => 'pending',
                'automation_source' => 'calendar_form',
                'automation_key' => "calendar-form-{$reminder['channel']}-{$minutes}",
            ]);
        }
    }
