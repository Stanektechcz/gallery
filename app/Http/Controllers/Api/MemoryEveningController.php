...
class MemoryEveningController extends Controller
{
    ...
    {
        $this->write($request);
        $evening = $this->evening($request, $uuid);
    ...
    {
        abort_if($request->user()->read_only_mode, 403, 'V režimu pouze pro čtení nelze večer se vzpomínkami měnit.');
    }

    private function available(): void
    {
        abort_unless(Schema::hasTable('memory_evenings') && Schema::hasTable('memory_evening_reflections'), 503, 'Pro večery se vzpomínkami dokončete databázové migrace.');
    }
}
