...
class PartnerCoordinationService
{
    ...
    private function planningItems(GallerySpace $space, Carbon $now): Collection
    {
        ...
        if (Schema::hasColumn('travel_inbox_items', 'assigned_to')) {
            $select = array_merge($select, ['item.assigned_to', 'assignee.name as assignee_name']);
        }
        ...
        if (Schema::hasColumn('travel_inbox_items', 'assigned_to')) {
            $query->leftJoin('users as assignee', 'assignee.id', '=', 'item.assigned_to');
        }
    ...
    private function tripDocuments(GallerySpace $space, Carbon $now): Collection
    {
        ...
        if (Schema::hasColumn('trip_document_checks', 'assigned_to')) {
            $select = array_merge($select, ['document.assigned_to', 'assignee.name as assignee_name']);
        }
    ...
    private function gifts(GallerySpace $space, User $viewer, Carbon $now): Collection
    {
        ...
        if (Schema::hasColumn('gift_ideas', 'assigned_to')) {
            $select = array_merge($select, ['gift.assigned_to', 'assignee.name as assignee_name']);
        }
    ...
    {
        if (! Schema::hasTable('trip_settlements')) {
            return collect();
        }
    ...
    {
        if ($unassigned > 0) {
            return ['code' => 'assign', 'title' => 'Domluvte odpovědnost', 'message' => $unassigned.' položek zatím neví, kdo je zařídí.'];
        }
        if ($members->count() < 2) {
            return null;
        }
        $low = $members->first(fn ($member) => in_array($member['check_in']['capacity'] ?? null, ['light', 'unavailable'], true) || (($member['check_in']['energy'] ?? 5) <= 2));
        if ($low && $low['open_actions'] > 0) {
            return ['code' => 'low_capacity', 'title' => 'Dnes zvolnit', 'message' => $low['name'].' hlásí nižší kapacitu. Zvažte přesun nedůležitých kroků.'];
        }
        $most = $members->sortByDesc('open_actions')->first();
        $least = $members->sortBy('open_actions')->first();
