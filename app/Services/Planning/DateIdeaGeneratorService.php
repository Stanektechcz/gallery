...
class DateIdeaGeneratorService
{
    ...
    private function compose(
        ...
        if ($openers->isEmpty() || $cores->isEmpty() || $closers->isEmpty()) {
            return null;
        }
        ...
        if ($anchor) {
            $blocks->splice(1, 0, [$this->placeBlock($anchor)]);
        }
        $foodBlock = $this->requestedFoodBlock($parameters['food'], $parameters['travel_scope'], $setting);
        if ($foodBlock && ! $blocks->contains('key', $foodBlock['key'])) {
            $blocks->splice(-1, 0, [$foodBlock]);
        }
        ...
        if ($estimatedCost > $parameters['budget_max']) {
            return null;
        }
        ...
        if ($parameters['duration'] === 'weekend') {
            $estimatedMinutes = max(1440, $baseMinutes);
        }
    ...
    {
        if (in_array($food, ['any', 'none'], true)) {
            return null;
        }
        $item = match ($food) {
    ...
    private function forecast(array $parameters): ?array
    {
        ...
        $lat = $parameters['destination']['latitude'] ?? null;
        $lng = $parameters['destination']['longitude'] ?? null;
        if (! $parameters['weather_aware'] || ! $date || $lat === null || $lng === null) {
            return null;
        }
    ...
    private function suggestStart(GallerySpace $space, array $parameters, int $minutes): Carbon
    {
        ...
        for ($offset = 0; $offset < 35; $offset++) {
            ...
            if (! $parameters['preferred_date'] && ! in_array($day->dayOfWeek, [Carbon::FRIDAY, Carbon::SATURDAY, Carbon::SUNDAY], true)) {
                continue;
            }
    ...
    {
        $scope = match ($parameters['travel_scope']) {
            'home' => 'doma', 'nearby' => 'v blízkém okolí', 'city' => 'po městě', 'day_trip' => 'jako jednodenní výlet', 'weekend' => 'jako víkendovou cestu'
        };
    ...
    {
        if (in_array($mode, ['walk', 'bike'], true) || $scope === 'home') {
            return 0;
        }
