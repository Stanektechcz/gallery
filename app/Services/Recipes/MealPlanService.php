    ...
    private function payload(int $spaceId, User $viewer, Collection $meals, string $currency, ?int $eventId, ?int $tripId): array
    {
        ...
        $mealPayload = $meals->map(function (PlannedMeal $meal) use (&$shopping, $currency) {
            ...
            foreach ($recipe->ingredients as $ingredient) {
                ...
                if ($quantity !== null) {
                    $shopping[$key]['quantity'] += $quantity;
                    $shopping[$key]['has_numeric_quantity'] = true;
                }
                if ($ingredient->quantity_note) {
                    $shopping[$key]['quantity_notes'][] = $ingredient->quantity_note;
                }
                if ($ingredient->preparation) {
                    $shopping[$key]['preparations'][] = $ingredient->preparation;
                }
