...
class SharedExpenseSettlementService
{
        ...
        return $currencies->map(function (string $currency) use ($expenses, $members, $memberIds) {
            ...
            foreach ($expenses->where('currency', $currency) as $expense) {
                ...
                foreach ($shares as $share) {
                    if (isset($balances[(int) ($share['user_id'] ?? 0)])) {
                        $balances[(int) $share['user_id']]['owed'] += (float) $share['amount'];
                    }
                }
            }
            foreach ($balances as &$balance) {
                ...
                $balance['balance'] = round($balance['paid'] - $balance['owed'], 2);
            } unset($balance);
    ...
    private function equal(float $amount, array $memberIds): array
    {
        ...
        $extra = $cents - $base * count($memberIds);
    ...
    {
        $creditors = array_values(array_filter($balances, fn ($row) => $row['balance'] > 0.004));
        $debtors = array_values(array_filter($balances, fn ($row) => $row['balance'] < -0.004));
        $result = [];
        foreach ($debtors as &$debtor) {
            foreach ($creditors as &$creditor) {
                ...
                $result[] = ['from_user_id' => $debtor['user_id'], 'from_name' => $debtor['name'], 'to_user_id' => $creditor['user_id'], 'to_name' => $creditor['name'], 'amount' => $amount, 'currency' => $currency];
