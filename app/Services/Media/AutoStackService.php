...
class AutoStackService
{
    public function candidates(int $spaceId, int $limit = 2000): Collection
    {
        ...
        foreach ($items as $item) {
            ...
            if ($current->count() >= 2) {
                $groups->push($this->formatGroup($current));
            }
    ...
    {
        return $items->sortByDesc(fn (MediaItem $item) => ($item->is_favorite ? 10000 : 0)
    ...
    private function belongsTogether(MediaItem $previous, MediaItem $current): bool
    {
        ...
        if ($sameBase && ($previous->is_raw || $current->is_raw || $previous->extension !== $current->extension)) {
            return true;
        }

        $seconds = abs($previous->taken_at->diffInSeconds($current->taken_at, false));
        if ($seconds > 3 || ! $previous->camera_model || $previous->camera_model !== $current->camera_model) {
            return false;
        }
