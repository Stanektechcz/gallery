...
class IntegrationSetting extends Model
{
    ...
    {
        return ['is_enabled' => 'boolean', 'last_tested_at' => 'datetime'];
    }

    public function config(): array
    {
        if (! $this->encrypted_config) {
            return [];
        }
        try {
            return json_decode(Crypt::decryptString($this->encrypted_config), true, 512, JSON_THROW_ON_ERROR);
        } catch (\Throwable) {
            return [];
        }
    }
