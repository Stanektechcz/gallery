        ...
        <AppLayout>
            ...
            <div className="p-4 max-w-2xl mx-auto pb-8">
                <div className="flex items-center gap-3 mb-6">
                    ...
                    <div>
                        <h1 className="text-lg font-semibold text-[var(--color-text-primary)]">Recovery Center</h1>
                ...
                <div className="bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-xl p-4 mb-4">
                    <h2 className="text-sm font-semibold text-[var(--color-text-primary)] mb-3">Stav systému</h2>
                    <div className="space-y-2">
                        {checks.map(c => (
                            <div key={c.label} className="flex items-center gap-3">
                                ...
                                <span className="text-sm text-[var(--color-text-primary)] flex-1">{c.label}</span>
                ...
                {drive_info && (
                    <div className="bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-xl p-4 mb-4">
                        <h2 className="text-sm font-semibold text-[var(--color-text-primary)] mb-3">Google Drive</h2>
                        <div className="space-y-1.5 text-xs">
                            <div className="flex justify-between"><span className="text-[var(--color-text-secondary)]">Účet</span><span className="text-[var(--color-text-primary)]">{drive_info.email}</span></div>
                            ...
                            {drive_info.quota_used && drive_info.quota_total && (
                                <div className="flex justify-between"><span className="text-[var(--color-text-secondary)]">Úložiště</span>
                                    <span className="text-[var(--color-text-primary)]">{fmt(drive_info.quota_used)} / {fmt(drive_info.quota_total)}</span>
                                </div>
                            )}
                            {drive_info.last_ok && (
                                <div className="flex justify-between"><span className="text-[var(--color-text-secondary)]">Poslední OK</span>
                                    <span className="text-[var(--color-text-primary)]">{new Date(drive_info.last_ok).toLocaleString('cs-CZ')}</span>
                                </div>
                            )}
                        </div>
                    </div>
                ...
                {/* Media stats */}
                <div className="bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-xl p-4 mb-4">
                    <h2 className="text-sm font-semibold text-[var(--color-text-primary)] mb-3">Média</h2>
                ...
                {stackPreview && (
                    <div className="mb-4 rounded-2xl border border-[var(--color-border)] bg-[var(--color-bg-card)] p-4">
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            ...
                            <button onClick={applyStacks} disabled={!stackPreview.count || stacking} className="min-h-10 shrink-0 rounded-lg bg-[var(--color-accent)] px-4 text-xs font-medium text-[var(--color-text-primary)] disabled:opacity-40">{stacking ? 'Seskupuji…' : stackPreview.count ? `Seskupit ${stackPreview.count} sérií` : 'Vše seskupeno'}</button>
                        </div>
                    </div>
                )}

                <div className="bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-xl p-4">
                    <h2 className="text-sm font-semibold text-[var(--color-text-primary)] mb-3">Opravné akce</h2>
