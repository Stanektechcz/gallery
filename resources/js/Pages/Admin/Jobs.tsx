    ...
    return (
        <AppLayout>
            ...
            <main className="mx-auto max-w-6xl p-4 sm:p-6">
                ...
                <h1 className="mt-1 text-2xl font-bold text-white sm:text-3xl">Fronta úloh</h1>
                ...
                <section className="mt-6">
                    <div className="mb-3 flex items-center gap-2">
                        ...
                        <h2 className="font-semibold text-white">Čekající ({pending.length})</h2>
                    </div>
                            ...
                            <tbody>
                                {pending.map(job => (
                                    <tr key={job.id} className="border-b border-[var(--color-border)] last:border-0">
                                        <td className="px-4 py-3 text-white">{jobName(job.payload)}</td>
                ...
                <section className="mt-7">
                    <div className="mb-3 flex items-center gap-2">
                        ...
                        <h2 className="font-semibold text-white">Selhané ({failed.length})</h2>
                    </div>
                    <div className="space-y-2">
                        {failed.map(job => (
                            <div key={job.id} className="rounded-xl border border-red-400/20 bg-red-500/5 p-3">
                                <div className="flex flex-wrap items-baseline justify-between gap-2">
                                    <p className="font-medium text-white">{jobName(job.payload)}</p>
