                ...
                <section className="mt-6 grid gap-4 md:grid-cols-2">
                    <Link href="/admin/integrations" className="group rounded-2xl border border-[var(--color-accent)]/30 bg-[var(--color-accent)]/5 p-5 transition hover:bg-[var(--color-accent)]/10">
                        <div className="flex items-start gap-3">
                            ...
                            <div><h2 className="font-semibold text-[var(--color-text-primary)]">Integrace a API klíče</h2><p className="mt-1 text-sm leading-relaxed text-[var(--color-text-secondary)]">GoCardless/Revolut, TMDB, Cinema City, mapy, trasy, počasí a měnové kurzy.</p><span className="mt-3 inline-block text-xs font-medium text-[var(--color-accent)]">Otevřít konfiguraci →</span></div>
                        </div>
                    </Link>
                    <Link href="/settings/storage/google" className="group rounded-2xl border border-[var(--color-border)] bg-[var(--color-bg-card)] p-5 transition hover:bg-[var(--color-surface-hover)]">
                        <div className="flex items-start gap-3">
                            <span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-blue-500/10 text-blue-200"><HardDrive size={21}/></span>
                            <div><h2 className="font-semibold text-[var(--color-text-primary)]">Google Drive</h2><p className="mt-1 text-sm text-[var(--color-text-secondary)]">{connection?.account_email ?? 'Úložiště zatím není připojeno'} · {connection?.connection_status ?? 'bez připojení'}</p><span className="mt-3 inline-block text-xs font-medium text-[var(--color-accent)]">Spravovat úložiště →</span></div>
                        </div>
                    </Link>
                    <Link href="/finances#connection" className="group rounded-2xl border border-[var(--color-border)] bg-[var(--color-bg-card)] p-5 transition hover:bg-[var(--color-surface-hover)]">
                        <div className="flex items-start gap-3">
                            ...
                            <div><h2 className="font-semibold text-[var(--color-text-primary)]">Finance a Revolut</h2><p className="mt-1 text-sm text-[var(--color-text-secondary)]">Připojené účty, importy, synchronizace a přehled transakcí.</p><span className="mt-3 inline-block text-xs font-medium text-[var(--color-accent)]">Otevřít finance →</span></div>
                        </div>
                    </Link>
                    <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-bg-card)] p-5">
                        <div className="flex items-start gap-3"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-violet-500/10 text-violet-200"><ShieldCheck size={21}/></span><div><h2 className="font-semibold text-[var(--color-text-primary)]">Fronta zpracování</h2><p className="mt-1 text-sm text-[var(--color-text-secondary)]">Čeká: {number(queue.pending)} · selhalo: {number(queue.failed)}</p><p className="mt-3 text-xs text-[var(--color-text-secondary)]">Fotografie, videa, metadata, náhledy a synchronizace integrací.</p></div></div>
                    </div>
                </section>

                <section className="mt-7">
                    <h2 className="text-lg font-semibold text-[var(--color-text-primary)]">Stav obsahu</h2>
                    <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
                        {[
                            ...
                            <div key={label} className="rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-card)] p-3">
                                ...
                                <p className="mt-3 text-xl font-semibold text-[var(--color-text-primary)]">{number(value)}</p>
