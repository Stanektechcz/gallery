...
import { podklady, ukoly } from '@/lib/cestina';
        ...
        <AppLayout>
            ...
            <div role="main" className="w-full p-4 sm:p-6 lg:p-8 space-y-5 pb-8">
                ...
                <div className="flex flex-col gap-3 pt-2 sm:flex-row sm:items-start sm:justify-between">
                    <div><h1 className="text-2xl font-bold text-[var(--color-text-primary)]">{data.greeting}, {data.user_name} {emoji}</h1><p className="mt-1 text-sm text-[var(--color-text-secondary)]">{new Date().toLocaleDateString('cs-CZ', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}{data.generated_at ? ` · aktualizováno ${new Date(data.generated_at).toLocaleTimeString('cs-CZ', { hour: '2-digit', minute: '2-digit' })}` : ''}</p></div>
                    <button type="button" onClick={refreshDashboard} disabled={dashboardRefreshing} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-xl border border-[var(--color-border)] px-3 text-sm text-[var(--color-text-primary)] hover:bg-[var(--color-surface-hover)] disabled:opacity-50"><RefreshCw size={15} className={dashboardRefreshing ? 'animate-spin' : ''}/>{dashboardRefreshing ? 'Aktualizuji…' : 'Aktualizovat'}</button>
                </div>
                ...
                {!data.upcoming_trip && !data.partner_hub?.next_event && nextActions.length === 0 && data.action_inbox.length === 0 && (data.partner_hub?.milestones?.length ?? 0) === 0 && (
                    <section className="rounded-3xl border border-dashed border-[var(--color-border)] bg-[var(--color-bg-card)] p-5 text-center">
                        ...
                        <h2 className="mt-3 font-semibold text-[var(--color-text-primary)]">Máte volno na nové plány</h2>
                        ...
                        <div className="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
                            <Link href="/calendar" className="inline-flex min-h-10 items-center justify-center rounded-xl bg-[var(--color-accent)] px-4 text-sm font-medium text-[var(--color-accent-contrast)]">Naplánovat akci</Link>
                ...
                {/* Karet s nabídkami tu bývá až tucet a na telefonu stály pod sebou přes
                    tisíc bodů — tři obrazovky jenom tohohle. Jsou to rovnocenné návrhy, ne
                    seznam s pořadím, takže je z nich pás, kterým se listuje do strany.
                    Karta je široká pětaosmdesát procent, aby z té další koukal kus a bylo
                    poznat, že se dá posunout. Od md výš zůstává mřížka o dvou sloupcích,
                    tam je místa dost. */}
                    ...
                    <CardStrip>
                        ...
                        {data.upcoming_trip && (
                            <DashCard icon={Route} label="Co následuje" href={`/trips/${data.upcoming_trip.id}/plan`} color="#14b8a6">
                                ...
                                {data.upcoming_trip.preparation && <div className={`mt-2 rounded-xl p-2 text-xs ${data.upcoming_trip.preparation.status==='attention'?'bg-amber-500/10 text-amber-100':'bg-[var(--color-surface-muted)] text-[var(--color-text-secondary)]'}`}><div className="flex items-center justify-between"><span>Příprava cesty</span><strong className={data.upcoming_trip.preparation.status==='ready'?'text-emerald-300':data.upcoming_trip.preparation.status==='attention'?'text-amber-300':'text-[var(--color-text-primary)]'}>{data.upcoming_trip.preparation.score}/100</strong></div>{data.upcoming_trip.preparation.actions[0]&&<p className="mt-1 text-[var(--color-text-primary)]">Další krok: {data.upcoming_trip.preparation.actions[0].title}</p>}{!data.upcoming_trip.preparation.actions[0]&&data.upcoming_trip.preparation.next_item&&<p className="mt-1">Následuje {data.upcoming_trip.preparation.next_item.title} · {new Date(data.upcoming_trip.preparation.next_item.at).toLocaleString('cs-CZ',{day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'})}</p>}{data.upcoming_trip.preparation.summary.risky_connections>0&&<p className="mt-1 text-amber-200">Prověřit {data.upcoming_trip.preparation.summary.risky_connections} těsných přestupů.</p>}</div>}
                        ...
                        {data.pinned_views?.length > 0 && (
                            <DashCard icon={Sparkles} label="Připnuté pohledy" color="#8b5cf6">
                                <div className="flex flex-wrap gap-2">{data.pinned_views.map(view => <Link key={view.id} href={`/search?view=${view.id}`} className="rounded-xl border border-[var(--color-border)] px-3 py-2 text-xs text-[var(--color-text-primary)] hover:border-[var(--color-accent)]">{view.icon ?? '✨'} {view.name}</Link>)}</div>
                            </DashCard>
                        )}
                    </CardStrip>
                )}
                {data.partner_hub?.milestones?.length > 0 && (
                    <section className="rounded-3xl border border-pink-500/20 bg-gradient-to-br from-pink-500/10 to-[var(--color-bg-card)] p-4 sm:p-5">
                        <div className="flex items-center justify-between"><div><p className="text-xs font-medium uppercase tracking-wider text-pink-200">Náš společný prostor</p><h2 className="mt-1 font-semibold text-[var(--color-text-primary)]">Blížící se osobní dny</h2></div><Heart size={20} className="text-pink-300" /></div>
                        <div className="mt-4 space-y-1">{data.partner_hub.milestones.map(item => <Link key={item.uuid} href="/milestones" className={`flex items-center justify-between rounded-xl px-3 py-2 text-sm text-white hover:bg-[var(--color-surface-muted)] ${item.is_highlighted ? 'bg-amber-500/10' : 'bg-[var(--color-surface-muted)]'}`}><span className="truncate">{item.icon} {item.title}</span><span className="ml-2 shrink-0 text-xs text-pink-200">{item.days_until === 0 ? 'dnes' : `za ${item.days_until} d.`}</span></Link>)}</div>
                    </section>
                )}

                <form onSubmit={addMilestone} className="flex flex-col gap-2 rounded-2xl border border-[var(--color-border)] bg-[var(--color-bg-card)] p-4 sm:flex-row sm:items-end"><div className="min-w-0 flex-1"><p className="text-xs font-medium text-[var(--color-text-primary)]">Přidat společný milník</p><p className="mt-1 text-xs text-[var(--color-text-secondary)]">Výročí se následně automaticky objeví v přehledu i kalendáři.</p></div><input required value={milestone.title} onChange={event => setMilestone({...milestone,title:event.target.value})} placeholder="Např. první společný výlet" className="min-h-10 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface-muted)] px-3 text-sm text-[var(--color-text-primary)]"/><input required type="date" value={milestone.occurred_on} onChange={event => setMilestone({...milestone,occurred_on:event.target.value})} className="min-h-10 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface-muted)] px-3 text-sm text-[var(--color-text-primary)]"/><button className="min-h-10 rounded-lg bg-[var(--color-accent)] px-4 text-sm text-[var(--color-accent-contrast)]">{milestoneSaved ? 'Uloženo ✓' : 'Přidat'}</button></form>
                <section className="rounded-3xl border border-[var(--color-border)] bg-[var(--color-bg-card)] p-5">
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        <div><p className="text-xs font-medium uppercase tracking-wider text-[var(--color-accent)]">Právě teď</p><h2 className="mt-1 font-semibold text-[var(--color-text-primary)]">Aktuální práce s galerií</h2><p className="mt-1 text-sm text-[var(--color-text-secondary)]">Historické vzpomínky najdete ve Vzpomínkách a Timeline. Dashboard ponechává jen živé kroky.</p></div>
                        <Link href="/timeline" className="inline-flex min-h-10 items-center justify-center rounded-xl border border-[var(--color-border)] px-4 text-sm text-[var(--color-text-primary)]">Otevřít Timeline</Link>
                    </div>
