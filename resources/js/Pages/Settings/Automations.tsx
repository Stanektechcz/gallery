    ...
    return <AppLayout><Head title="Automatizace"/><main className="w-full p-4 sm:p-6 lg:p-8">
        <div className="w-full">
            ...
            <div className="mt-3 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div><h1 className="flex items-center gap-3 text-2xl font-bold text-[var(--color-text-primary)]"><Power className="text-[var(--color-accent)]"/>Automatizace prostoru</h1><p className="mt-2 max-w-3xl text-sm text-[var(--color-text-secondary)]">Pravidla šetří údržbu, ale zůstávají vždy pod vaší kontrolou. Vypnuté pravidlo nic automaticky nemění.</p></div>
                <button type="button" onClick={() => void load()} disabled={!spaceId || loading} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-lg border border-[var(--color-border)] px-3 text-sm text-[var(--color-text-primary)] hover:bg-[var(--color-surface-hover)] disabled:opacity-50"><RefreshCw size={15} className={loading ? 'animate-spin' : ''}/>Obnovit</button>
            </div>
            <div className="mt-5 rounded-2xl border border-[var(--color-border)] bg-[var(--color-bg-card)] p-4 text-sm text-[var(--color-text-secondary)]"><span className="font-medium text-[var(--color-text-primary)]">Spravovaný prostor:</span> {spaceName || (spaceLoading ? 'načítám…' : 'není dostupný')}</div>
            ...
            <section className="mt-5 grid gap-4">
                {items.map(automation => <article key={automation.key} className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-bg-card)] p-4 sm:p-5">
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><CalendarCheck2 size={19} className="shrink-0 text-emerald-300"/><h2 className="text-lg font-semibold text-[var(--color-text-primary)]">{automation.title}</h2></div><p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--color-text-secondary)]">{automation.description}</p><dl className="mt-4 grid gap-2 sm:grid-cols-3"><div className="rounded-xl bg-[var(--color-surface-muted)] p-3"><dt className="flex items-center gap-1.5 text-[11px] uppercase tracking-wide text-[var(--color-text-secondary)]"><Clock3 size={13}/>Spouštění</dt><dd className="mt-1 text-sm text-[var(--color-text-primary)]">{automation.schedule}</dd></div><div className="rounded-xl bg-[var(--color-surface-muted)] p-3"><dt className="flex items-center gap-1.5 text-[11px] uppercase tracking-wide text-[var(--color-text-secondary)]"><Info size={13}/>Podmínka</dt><dd className="mt-1 text-sm text-[var(--color-text-primary)]">{automation.condition}</dd></div><div className="rounded-xl bg-[var(--color-surface-muted)] p-3"><dt className="text-[11px] uppercase tracking-wide text-[var(--color-text-secondary)]">Naposledy</dt><dd className="mt-1 text-sm text-[var(--color-text-primary)]">{dateTime(automation.last_run_at)}</dd></div></dl></div>
                        <label className="flex min-h-12 shrink-0 cursor-pointer items-center gap-3 rounded-xl border border-[var(--color-border)] px-3 text-sm text-[var(--color-text-primary)] hover:bg-[var(--color-surface-hover)]"><span>{automation.enabled ? 'Zapnuto' : 'Vypnuto'}</span><input type="checkbox" checked={automation.enabled} disabled={saving === automation.key} onChange={() => void toggle(automation)} className="h-5 w-5 accent-emerald-500 disabled:opacity-50" aria-label={`${automation.title}: ${automation.enabled ? 'vypnout' : 'zapnout'}`}/></label>
                    </div>
                </article>)}
