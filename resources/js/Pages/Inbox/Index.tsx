            ...
            {recentLifeEvents.length === 0 ? (
                ...
                <div className="mt-4 grid gap-2 lg:grid-cols-2">
                    {recentLifeEvents.map((event) => (
                        <Link key={event.uuid} href={event.href} className="flex min-h-14 items-center gap-3 rounded-xl border border-white/10 bg-black/10 p-3 hover:border-[var(--color-accent)]/50">
                            ...
                            <span className="min-w-0 flex-1"><span className="block text-[10px] uppercase tracking-wide text-[var(--color-text-secondary)]">{eventKindLabel[event.kind] ?? event.kind.replaceAll('.', ' · ')} · {eventSourceLabel(event.source)}</span><span className="block truncate text-sm font-medium text-white">{event.title}</span></span>
