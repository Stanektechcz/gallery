...
export default function AnniversaryAlbumIndex() {
    ...
    return <AppLayout><Head title="Výroční album"/><main className="w-full p-4 sm:p-6">
        ...
        {!spaceLoading && !loading && spaceId && !startedOn && <div className="mt-5 rounded-2xl border border-dashed border-pink-300/25 bg-pink-500/5 p-6 text-center"><p className="text-[var(--color-text-primary)]">Nejprve nastavte datum začátku vztahu.</p><p className="mt-1 text-sm text-[var(--color-text-secondary)]">Podle něj určíme období pro každý společný rok.</p><Link href="/gifts-anniversaries" className="mt-4 inline-flex min-h-10 items-center rounded-lg bg-pink-600 px-4 text-sm text-white">Nastavit výročí</Link></div>}
