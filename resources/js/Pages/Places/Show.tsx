                ...
                <div className="px-6 py-4 border-b border-[var(--color-border)] shrink-0">
                    <div className="flex items-start gap-4">
                        ...
                        <button onClick={() => router.visit('/places')}
                            className="p-1.5 text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] transition-colors shrink-0 mt-0.5">
                            <ArrowLeft size={18}/>
                        </button>
                        ...
                        <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 flex-wrap">
                                <h1 className="text-xl font-bold text-[var(--color-text-primary)]">{place.name}</h1>
                                ...
                                <span className="text-xs bg-[var(--color-accent)]/15 border border-[var(--color-accent)]/25 px-2 py-0.5 rounded-full text-[var(--color-accent-contrast)]">
                            ...
                            {(place.is_rain_friendly || place.is_accessible || place.is_photogenic || place.opens_early || place.personal_rating || place.review_average) && <div className="mt-3 flex flex-wrap gap-1.5 text-[10px]">{place.is_rain_friendly && <span className="rounded-full bg-sky-500/15 px-2 py-1 text-sky-200">🌧️ Vhodné při dešti</span>}{place.is_accessible && <span className="rounded-full bg-emerald-500/15 px-2 py-1 text-emerald-200">♿ Bezbariérové</span>}{place.is_photogenic && <span className="rounded-full bg-fuchsia-500/15 px-2 py-1 text-fuchsia-200">📸 Fotogenické</span>}{place.opens_early && <span className="rounded-full bg-amber-500/15 px-2 py-1 text-amber-200">🌅 Otevřeno brzy</span>}{(place.review_average ?? place.personal_rating) && <span className="rounded-full bg-yellow-500/15 px-2 py-1 text-yellow-200">★ {place.review_average ?? place.personal_rating}/5{place.review_count ? ` · ${place.review_count} návštěv` : ''}</span>}</div>}
                            {(place.estimated_visit_minutes || place.next_time_note || place.price_level) && <div className="mt-3 text-xs text-[var(--color-text-secondary)]">{place.estimated_visit_minutes && <span>⏱️ přibližně {place.estimated_visit_minutes} min</span>}{place.price_level && <span className="ml-3">💰 {'€'.repeat(place.price_level)}</span>}{place.next_time_note && <p className="mt-2 rounded-lg bg-[var(--color-surface-muted)] p-2">Příště: {place.next_time_note}</p>}</div>}
                            {showTripPlanner && <div className="mt-4 rounded-xl border border-[var(--color-accent)]/35 bg-[var(--color-bg-card)] p-3"><p className="text-xs font-semibold text-[var(--color-text-primary)]">Přidat do itineráře</p><p className="mt-1 text-[11px] text-[var(--color-text-secondary)]">Místo, souřadnice i poznámka pro příště se uloží jako blok vybraného dne.</p><div className="mt-3 grid gap-2 sm:grid-cols-3"><select value={selectedTripId} onChange={event => chooseTrip(event.target.value)} className="min-h-10 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-secondary)] px-2 text-xs text-[var(--color-text-primary)]"><option value="">Vyberte cestu</option>{trips.map(trip => <option key={trip.id} value={trip.id}>{trip.name} · {new Date(`${trip.start_date}T12:00:00`).toLocaleDateString('cs-CZ')}</option>)}</select><select disabled={!selectedTripId} value={selectedDayId} onChange={event => setSelectedDayId(event.target.value)} className="min-h-10 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-secondary)] px-2 text-xs text-[var(--color-text-primary)] disabled:opacity-50"><option value="">Vyberte den</option>{tripDays.map(day => <option key={day.id} value={day.id}>{day.title || 'Den'} · {new Date(`${day.date}T12:00:00`).toLocaleDateString('cs-CZ')}</option>)}</select><button disabled={!selectedDayId || planningTrip} onClick={addToTrip} className="min-h-10 rounded-lg bg-[var(--color-accent)] px-3 text-xs font-medium text-[var(--color-accent-contrast)] disabled:opacity-50">{planningTrip ? 'Přidávám…' : 'Přidat do dne'}</button></div>{trips.length === 0 && <p className="mt-2 text-[11px] text-[var(--color-text-secondary)]">Nejprve vytvořte cestu v plánování výletů.</p>}{tripPlanMessage && <p className={`mt-2 text-[11px] ${tripPlanMessage === 'Místo je v itineráři.' ? 'text-emerald-300' : 'text-red-300'}`}>{tripPlanMessage}</p>}</div>}
                        </div>

                        {/* Actions */}
                        <div className="flex gap-2 shrink-0">
                            <button onClick={toggleTripPlanner} className={`flex items-center gap-1.5 border text-xs px-3 py-1.5 rounded-lg transition-colors ${showTripPlanner ? 'border-[var(--color-accent)] text-[var(--color-text-primary)]' : 'border-[var(--color-border)] text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)]'}`}>
                            ...
                            {place.latitude && place.longitude && (
                                <button onClick={autoLink} disabled={autoLinking} title="Automaticky propojit fotky v GPS poloměru"
                                    className="flex items-center gap-1.5 border border-[var(--color-border)] text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] text-xs px-3 py-1.5 rounded-lg transition-colors disabled:opacity-40">
                                    <RefreshCw size={12} className={autoLinking ? 'animate-spin' : ''}/> Auto-link
                                </button>
                            )}
                            <button onClick={() => setEditMode(v => !v)}
                                className="flex items-center gap-1.5 border border-[var(--color-border)] text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] text-xs px-3 py-1.5 rounded-lg transition-colors">
                    ...
                    {editMode && (
                        <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
                            ...
                            <input value={editForm.website_url} onChange={e => setEditForm(p => ({ ...p, website_url: e.target.value }))} type="url"
                                ...
                                className="bg-[var(--color-bg-secondary)] border border-[var(--color-border)] rounded-lg px-3 py-2 text-sm text-[var(--color-text-primary)] placeholder-[var(--color-text-secondary)] outline-none focus:border-[var(--color-accent)]"/>
                            <div className="flex items-center gap-2">
                                <input value={editForm.radius_meters} onChange={e => setEditForm(p => ({ ...p, radius_meters: e.target.value }))} type="number" min="10" max="50000"
                                    ...
                                    className="flex-1 bg-[var(--color-bg-secondary)] border border-[var(--color-border)] rounded-lg px-3 py-2 text-sm text-[var(--color-text-primary)] placeholder-[var(--color-text-secondary)] outline-none focus:border-[var(--color-accent)]"/>
                            ...
                            <select value={editForm.lifecycle_status} onChange={event => setEditForm(p => ({...p,lifecycle_status:event.target.value as LifecycleStatus}))} className="col-span-full min-h-10 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-secondary)] px-3 text-xs text-[var(--color-text-primary)]"><option value="idea">Nápad</option><option value="planned">Plánujeme</option><option value="visited">Navštíveno</option><option value="favorite">Oblíbené</option><option value="avoid">Už ne</option></select>
                            <div className="col-span-full grid grid-cols-3 gap-2"><input value={editForm.personal_rating} onChange={event => setEditForm(p => ({...p,personal_rating:event.target.value}))} type="number" min="1" max="5" placeholder="Hodnocení 1–5" className="min-h-10 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-secondary)] px-3 text-xs text-[var(--color-text-primary)]"/><input value={editForm.price_level} onChange={event => setEditForm(p => ({...p,price_level:event.target.value}))} type="number" min="1" max="4" placeholder="Cena 1–4" className="min-h-10 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-secondary)] px-3 text-xs text-[var(--color-text-primary)]"/><input value={editForm.estimated_visit_minutes} onChange={event => setEditForm(p => ({...p,estimated_visit_minutes:event.target.value}))} type="number" min="5" max="1440" placeholder="Délka v min" className="min-h-10 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-secondary)] px-3 text-xs text-[var(--color-text-primary)]"/></div>
                            <textarea value={editForm.next_time_note} onChange={event => setEditForm(p => ({...p,next_time_note:event.target.value}))} placeholder="Co příště objednat nebo nezapomenout…" rows={2} className="col-span-full rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-secondary)] px-3 py-2 text-sm text-[var(--color-text-primary)]"/>
                            <div className="col-span-full flex gap-2">
                                <button onClick={saveEdit} className="bg-[var(--color-accent)] text-[var(--color-accent-contrast)] text-sm px-4 py-1.5 rounded-lg hover:opacity-90">Uložit</button>
                                <button onClick={() => setEditMode(false)} className="border border-[var(--color-border)] text-[var(--color-text-secondary)] text-sm px-4 py-1.5 rounded-lg hover:text-[var(--color-text-primary)]">Zrušit</button>
                            </div>
                        </div>
                    )}
                        ...
                        <div className="w-64 shrink-0 border-r border-[var(--color-border)] relative">
                            ...
                            <div className="absolute bottom-2 right-2 z-[1000]">
                                <a href={`https://www.google.com/maps/search/?api=1&query=${place.latitude},${place.longitude}`}
                                    ...
                                    className="flex items-center gap-1 text-[10px] bg-[var(--color-bg-card)]/90 backdrop-blur-sm text-[var(--color-text-primary)] px-2 py-1 rounded hover:opacity-90">
                    ...
                    <div className="flex-1 flex flex-col overflow-hidden">
                        ...
                        <div className="flex gap-4 px-6 border-b border-[var(--color-border)] shrink-0">
                            <button onClick={() => setTab('photos')}
                                className={`flex items-center gap-1.5 py-3 text-sm border-b-2 transition-colors ${tab === 'photos' ? 'border-[var(--color-accent)] text-[var(--color-text-primary)]' : 'border-transparent text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)]'}`}>
                                <Camera size={14}/> Fotografie ({place.photo_count})
                            </button>
                            <button onClick={() => setTab('albums')}
                                className={`flex items-center gap-1.5 py-3 text-sm border-b-2 transition-colors ${tab === 'albums' ? 'border-[var(--color-accent)] text-[var(--color-text-primary)]' : 'border-transparent text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)]'}`}>
                                    ...
                                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                                        {albums.map(album => (
                                            <a key={album.uuid} href={`/albums/${album.uuid}`}
                                                ...
                                                <div className="p-2.5">
                                                    <p className="text-xs font-medium text-[var(--color-text-primary)] truncate">{album.title}</p>
...
function PlaceVisitPanel({ placeId }: { placeId:number }) {
    type Plan = { uuid:string; state:string; planned_for?:string|null; visited_on?:string|null; reservation_reference?:string|null; reservation_url?:string|null; notes?:string|null; calendar_event_uuid?:string|null; };
    ...
    const createMemory = async (plan:Plan) => { setMemoryUuid(plan.uuid); setMessage(''); try { if(plan.calendar_event_uuid)await axios.post(`/api/v1/calendar/events/${plan.calendar_event_uuid}/shared-memory`);else await axios.post(`/api/v1/places/${placeId}/plans/${plan.uuid}/shared-memory`);setMessage('Společná vzpomínka, návštěva, album i kalendář jsou propojené.');load(); } catch (error:any) { setMessage(error?.response?.data?.message ?? 'Vzpomínku se nepodařilo vytvořit.'); } finally { setMemoryUuid(null); } };
    return <section className="border-b border-[var(--color-border)] bg-[var(--color-bg-card)]/50 px-6 py-4"><div className="w-full"><div className="flex flex-wrap items-end justify-between gap-3"><div><h2 className="font-semibold text-[var(--color-text-primary)]">Návštěvy a rezervace</h2><p className="mt-1 text-xs text-[var(--color-text-secondary)]">Naplánování vytvoří společnou akci v kalendáři; po návštěvě se uzavře i ona.</p></div></div><form onSubmit={add} className="mt-3 grid gap-2 sm:grid-cols-4"><input required type="date" value={form.planned_for} onChange={event => setForm({...form,planned_for:event.target.value})} className="min-h-10 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface-muted)] px-2 text-sm text-[var(--color-text-primary)]"/><input value={form.reservation_reference} onChange={event => setForm({...form,reservation_reference:event.target.value})} placeholder="Kód rezervace" className="min-h-10 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface-muted)] px-2 text-sm text-[var(--color-text-primary)]"/><input type="url" value={form.reservation_url} onChange={event => setForm({...form,reservation_url:event.target.value})} placeholder="Odkaz https://" className="min-h-10 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface-muted)] px-2 text-sm text-[var(--color-text-primary)]"/><button disabled={saving} className="min-h-10 rounded-lg bg-[var(--color-accent)] px-3 text-sm text-[var(--color-accent-contrast)] disabled:opacity-50">{saving ? 'Ukládám…' : 'Naplánovat návštěvu'}</button></form>{plans.length > 0 && <div className="mt-3 flex flex-wrap gap-2">{plans.slice(0,5).map(plan => <div key={plan.uuid} className="rounded-lg border border-[var(--color-border)] bg-[var(--color-surface-muted)] px-3 py-2 text-xs text-[var(--color-text-secondary)]"><span className="text-[var(--color-text-primary)]">{plan.state === 'visited' ? '✓ Navštíveno' : plan.state === 'cancelled' ? 'Zrušeno' : `📅 ${plan.planned_for}`}</span>{plan.reservation_reference && <span className="ml-2">· {plan.reservation_reference}</span>}{plan.reservation_url && <a href={plan.reservation_url} target="_blank" rel="noreferrer" className="ml-2 text-[var(--color-accent)]">Rezervace</a>}{plan.state === 'planned' && <button onClick={() => visit(plan)} className="ml-2 text-emerald-300">Označit jako navštívené</button>}{plan.state === 'visited' && <button disabled={memoryUuid === plan.uuid} onClick={() => createMemory(plan)} className="ml-2 text-pink-200 disabled:opacity-50">{memoryUuid === plan.uuid ? 'Ukládám…' : 'Uložit vzpomínku'}</button>}</div>)}</div>}{message && <p className={`mt-2 text-xs ${message.startsWith('Návštěva je') || message.startsWith('Společná') ? 'text-emerald-300' : 'text-red-300'}`}>{message}</p>}</div></section>;
}
...
function PlaceWishlistPanel({ placeId }: { placeId:number }) {
    ...
    return <section className="border-b border-[var(--color-border)] bg-pink-500/5 px-6 py-4"><div className="mx-auto flex max-w-5xl flex-col gap-3 sm:flex-row sm:items-center"><div className="min-w-0 flex-1"><h2 className="font-semibold text-[var(--color-text-primary)]">Nápad na příště</h2><p className="mt-1 text-xs text-[var(--color-text-secondary)]">Místo, jeho fotky, GPS i vaše poznámka se uloží do společného přání a odtud do kalendáře.</p></div>{lists.length > 0 ? <div className="flex flex-col gap-2 sm:flex-row"><select value={selected} onChange={event => setSelected(event.target.value)} className="min-h-10 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-secondary)] px-2 text-sm text-[var(--color-text-primary)]">{lists.map(list => <option key={list.uuid} value={list.uuid}>{list.title}</option>)}</select><button disabled={saving} onClick={add} className="min-h-10 rounded-lg border border-pink-400/40 px-3 text-sm text-pink-100 disabled:opacity-50">{saving ? 'Přidávám…' : 'Přidat do přání'}</button></div> : <a href="/planning" className="text-sm text-pink-200 underline">Vytvořit společný seznam přání</a>}</div>{message && <p className={`mx-auto mt-2 max-w-5xl text-xs ${message.startsWith('Místo je') ? 'text-emerald-300' : 'text-red-300'}`}>{message}</p>}</section>;
}
