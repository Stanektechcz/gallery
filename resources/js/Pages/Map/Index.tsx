                ...
                <div className="px-4 py-3 border-b border-[var(--color-border)] flex items-center justify-between shrink-0">
                    ...
                    <div className="flex items-center gap-3 text-xs text-[var(--color-text-secondary)]">
                        <button onClick={() => setShowPhotos(v=>!v)}
                            className={`flex items-center gap-1 px-2 py-2 sm:py-1 rounded-lg transition-colors ${showPhotos ? 'bg-[var(--color-accent)] text-[var(--color-accent-contrast)]' : 'hover:text-[var(--color-text-primary)]'}`}>
                            <Image size={11}/> Fotky ({points.length})
                        </button>
                        <button onClick={() => setShowAlbums(v=>!v)}
                            className={`flex items-center gap-1 px-2 py-2 sm:py-1 rounded-lg transition-colors ${showAlbums ? 'bg-green-500 text-white' : 'hover:text-[var(--color-text-primary)]'}`}>
