                ...
                <form onSubmit={search} className="relative z-30">
                    <div className="flex flex-col gap-2 sm:flex-row">
                        ...
                        <div className="flex gap-2">
                            <button type="button" onClick={() => setShowFilters(value => !value)} className={`relative flex min-h-12 flex-1 items-center justify-center gap-2 rounded-2xl border px-4 text-sm sm:min-h-14 sm:flex-none ${showFilters ? 'border-[var(--color-accent)] bg-[var(--color-accent)]/15 text-[var(--color-text-primary)]' : 'border-[var(--color-border)] bg-[var(--color-bg-card)] text-[var(--color-text-secondary)]'}`}><Filter size={16} /> Filtry{activeCount > 0 && <span className="rounded-full bg-[var(--color-accent)] px-1.5 text-[10px] text-[var(--color-text-primary)]">{activeCount}</span>}</button>
                            <button type="submit" className="min-h-12 flex-1 rounded-2xl bg-[var(--color-accent)] px-6 text-sm font-semibold text-[var(--color-text-primary)] sm:min-h-14 sm:flex-none">Hledat</button>
                        </div>
                    </div>
                </form>
...
function ViewNotice({ icon, text, href }: { icon: string; text: string; href: string }) { return <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-bg-card)] p-8 text-center"><span className="text-4xl">{icon}</span><p className="mx-auto mt-3 max-w-md text-sm text-[var(--color-text-secondary)]">{text}</p><Link href={href} className="mt-4 inline-flex min-h-11 items-center rounded-xl bg-[var(--color-accent)] px-4 text-sm text-[var(--color-text-primary)]">Otevřít pohled</Link></div>; }
