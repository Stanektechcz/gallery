                    ...
                    <div className="flex flex-col gap-2 sm:flex-row">
                        <div className="relative flex-1">
                            ...
                            <input value={filters.q} onChange={event => setFilters(previous => ({ ...previous, q: event.target.value }))}
                                ...
                                className="min-h-14 w-full rounded-2xl border border-[var(--color-border)] bg-[var(--color-bg-card)] pl-12 pr-12 text-base text-[var(--color-text-primary)] shadow-lg outline-none transition focus:border-[var(--color-accent)]" />
                            {filters.q && <button type="button" onClick={() => setFilters(previous => ({ ...previous, q: '' }))} className="absolute right-3 top-1/2 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-xl text-[var(--color-text-secondary)] hover:bg-[var(--color-surface-hover)]"><X size={16} /></button>}
                            {showSuggestions && suggestions.length > 0 && (
                                <div className="absolute left-0 right-0 top-full mt-2 overflow-hidden rounded-2xl border border-[var(--color-border)] bg-[var(--color-bg-card)] shadow-2xl">
                                    ...
                                    {suggestions.map(suggestion => suggestion.url ? (
                                        <Link key={`${suggestion.type}-${suggestion.id}`} href={suggestion.url} className="flex min-h-12 items-center gap-3 px-4 text-sm text-[var(--color-text-primary)] hover:bg-[var(--color-surface-hover)]"><span>{suggestion.icon}</span><span className="flex-1">{suggestion.label}</span><span className="text-[10px] uppercase text-[var(--color-text-secondary)]">{suggestion.type}</span></Link>
                                    ) : (
                                        <button key={`${suggestion.type}-${suggestion.id}`} type="button" onClick={() => applyPatch({ ...suggestion.filters, q: suggestion.label })} className="flex min-h-12 w-full items-center gap-3 px-4 text-left text-sm text-[var(--color-text-primary)] hover:bg-[var(--color-surface-hover)]"><span>{suggestion.icon}</span><span className="flex-1">{suggestion.label}</span><span className="text-[10px] uppercase text-[var(--color-text-secondary)]">{suggestion.type}</span></button>
                                    ))}
                                </div>
                            )}
                        </div>
                        <div className="flex gap-2">
                            <button type="button" onClick={() => setShowFilters(value => !value)} className={`relative flex min-h-12 flex-1 items-center justify-center gap-2 rounded-2xl border px-4 text-sm sm:min-h-14 sm:flex-none ${showFilters ? 'border-[var(--color-accent)] bg-[var(--color-accent)]/15 text-[var(--color-accent-contrast)]' : 'border-[var(--color-border)] bg-[var(--color-bg-card)] text-[var(--color-text-secondary)]'}`}><Filter size={16} /> Filtry{activeCount > 0 && <span className="rounded-full bg-[var(--color-accent)] px-1.5 text-[10px] text-[var(--color-accent-contrast)]">{activeCount}</span>}</button>
                            <button type="submit" className="min-h-12 flex-1 rounded-2xl bg-[var(--color-accent)] px-6 text-sm font-semibold text-[var(--color-accent-contrast)] sm:min-h-14 sm:flex-none">Hledat</button>
                        </div>
                    </div>
                </form>

                <div className="mt-3 flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                    {QUICK_FILTERS.map(item => <button key={item.label} onClick={() => applyPatch(item.patch as Partial<SearchFilters>)} className="flex min-h-10 shrink-0 items-center gap-2 rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-card)] px-3 text-xs text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)]"><span>{item.icon}</span>{item.label}</button>)}
                </div>

                {(viewsQuery.data?.length ?? 0) > 0 && (
                    <section className="mt-3 rounded-2xl border border-[var(--color-border)] bg-[var(--color-bg-card)]/60 p-3">
                        <div className="mb-2 flex items-center justify-between"><p className="text-xs font-semibold text-[var(--color-text-primary)]">Vaše pohledy</p><span className="text-[10px] text-[var(--color-text-secondary)]">Jedna kolekce, různé zobrazení</span></div>
                        <div className="flex gap-2 overflow-x-auto scrollbar-hide">
                            {viewsQuery.data?.map(view => <div key={view.id} className="flex shrink-0 overflow-hidden rounded-xl border border-[var(--color-border)]"><button onClick={() => applyView(view)} className="flex min-h-10 items-center gap-2 px-3 text-xs text-[var(--color-text-primary)] hover:bg-[var(--color-surface-hover)]"><span>{view.icon ?? '✨'}</span>{view.name}</button><button onClick={() => togglePin(view)} className={`min-w-9 border-l border-[var(--color-border)] ${view.is_pinned ? 'text-[var(--color-accent)]' : 'text-[var(--color-text-secondary)]'}`}><Pin size={12} className="mx-auto" /></button></div>)}
                        </div>
                    </section>
                )}
                    ...
                    <section className="mt-6">
                        <div className="mb-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <button onClick={() => setShowSave(true)} className="flex min-h-10 items-center gap-2 rounded-xl border border-[var(--color-border)] px-3 text-xs text-[var(--color-text-primary)]"><BookmarkPlus size={14} /> Uložit</button>
                            </div>
                        </div>

                        {facets && <div className="mb-4 flex gap-2 overflow-x-auto scrollbar-hide">{[['Fotografie', facets.photos], ['Videa', facets.videos], ['Oblíbené', facets.favorites], ['S GPS', facets.with_gps]].map(([label, count]) => <span key={String(label)} className="shrink-0 rounded-lg bg-[var(--color-surface-muted)] px-2.5 py-1 text-[10px] text-[var(--color-text-secondary)]">{label} · {count}</span>)}</div>}

                        {results.length > 0 ? viewType === 'table' ? <ResultList results={results} /> : viewType === 'map' ? <ViewNotice icon="🗺️" text="Mapový pohled používá stejnou uloženou kolekci. Otevřete ji na mapě pro prostorové clustery." href="/map" /> : viewType === 'calendar' ? <ViewNotice icon="📅" text="Kalendář zobrazí stejnou kolekci podle data pořízení." href="/calendar" /> : <ResultGrid results={results} /> : !searchQuery.isLoading && <div className="rounded-2xl border border-dashed border-[var(--color-border)] py-16 text-center"><Search className="mx-auto mb-3 text-[var(--color-text-secondary)]" /><p className="text-sm text-[var(--color-text-primary)]">Nic jsme nenašli</p><p className="mt-1 text-xs text-[var(--color-text-secondary)]">Zkuste ubrat filtr nebo použít obecnější popis.</p></div>}
                    </section>
                )}

                {showSave && <div className="fixed inset-0 z-[700] flex items-end justify-center bg-black/60 p-0 sm:items-center sm:p-4"><div className="w-full max-w-sm rounded-t-3xl border border-[var(--color-border)] bg-[var(--color-bg-card)] p-5 sm:rounded-3xl"><h2 className="font-semibold text-[var(--color-text-primary)]">Uložit jako pohled</h2><p className="mt-1 text-xs text-[var(--color-text-secondary)]">Filtry i způsob zobrazení zůstanou spolu.</p><input autoFocus value={viewName} onChange={e => setViewName(e.target.value)} placeholder="Např. Nejlepší fotky z cest" className="mt-4 min-h-12 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-secondary)] px-3 text-sm text-[var(--color-text-primary)] outline-none" /><div className="mt-4 flex gap-2"><button onClick={() => setShowSave(false)} className="min-h-11 flex-1 rounded-xl border border-[var(--color-border)] text-sm text-[var(--color-text-secondary)]">Zrušit</button><button onClick={saveView} disabled={!viewName.trim() || savingView} className="min-h-11 flex-1 rounded-xl bg-[var(--color-accent)] text-sm text-[var(--color-accent-contrast)] disabled:opacity-40">{savingView ? 'Ukládám…' : 'Uložit'}</button></div></div></div>}
            </div>
        </AppLayout>
    );
}

function FilterSelect({ label, value, onChange, options }: { label: string; value: string; onChange: (value: string) => void; options: string[][] }) { return <label className="text-xs text-[var(--color-text-secondary)]">{label}<select value={value} onChange={e => onChange(e.target.value)} className="mt-1 min-h-11 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-secondary)] px-3 text-xs text-[var(--color-text-primary)]">{options.map(([key, text]) => <option key={key} value={key}>{text}</option>)}</select></label>; }
function FilterInput({ label, value, onChange, type = 'text', placeholder }: { label: string; value: string; onChange: (value: string) => void; type?: string; placeholder?: string }) { return <label className="text-xs text-[var(--color-text-secondary)]">{label}<input type={type} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} className="mt-1 min-h-11 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-secondary)] px-3 text-xs text-[var(--color-text-primary)]" /></label>; }
function ViewButton({ active, onClick, icon, label }: { active: boolean; onClick: () => void; icon: React.ReactNode; label: string }) { return <button onClick={onClick} title={label} className={`flex min-h-10 min-w-10 items-center justify-center border-r border-[var(--color-border)] last:border-0 ${active ? 'bg-[var(--color-accent)] text-[var(--color-accent-contrast)]' : 'text-[var(--color-text-secondary)]'}`}>{icon}</button>; }
...
function ResultList({ results }: { results: any[] }) { return <div className="overflow-hidden rounded-2xl border border-[var(--color-border)]">{results.map(item => <Link key={item.id} href={`/media/${item.uuid}`} className="flex min-h-16 items-center gap-3 border-b border-[var(--color-border)] px-3 last:border-0 hover:bg-[var(--color-surface-hover)]"><div className="h-12 w-12 overflow-hidden rounded-lg bg-[var(--color-bg-secondary)]">{item.variants?.[0]?.url && <img src={item.variants[0].url} alt="" className="h-full w-full object-cover" />}</div><div className="min-w-0 flex-1"><p className="truncate text-sm text-[var(--color-text-primary)]">{item.display_title || item.original_filename}</p><p className="text-xs text-[var(--color-text-secondary)]">{item.taken_at ? takenAtDate(item.taken_at)!.toLocaleDateString('cs-CZ') : 'Bez data'} · {item.media_type === 'video' ? 'Video' : 'Fotografie'}</p></div>{item.rating > 0 && <span className="text-xs text-amber-400">{'★'.repeat(item.rating)}</span>}</Link>)}</div>; }
function ViewNotice({ icon, text, href }: { icon: string; text: string; href: string }) { return <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-bg-card)] p-8 text-center"><span className="text-4xl">{icon}</span><p className="mx-auto mt-3 max-w-md text-sm text-[var(--color-text-secondary)]">{text}</p><Link href={href} className="mt-4 inline-flex min-h-11 items-center rounded-xl bg-[var(--color-accent)] px-4 text-sm text-[var(--color-accent-contrast)]">Otevřít pohled</Link></div>; }
