...
export default function StatementImport({ budget, categories, onDone, onCancel }: {
    ...
    const read = async (file: File) => {
        ...
        try {
            ...
            const nactene = (data.rows ?? []) as Radek[];
            ...
            setNote(nactene.length === 0
                ...
                : `Načteno ${nactene.length} položek${data.skipped ? `, ${data.skipped} přeskočeno` : ''}${data.duplicates ? `, ${data.duplicates} už v rozpočtu je` : ''}.`);
        } catch (problem: any) {
        ...
        try {
            await axios.post(`/api/v1/rozpocty/${budget.uuid}/vypis`, {
                rows: [...chosen].sort((a, b) => a - b).map(i => ({
                    ...
                    note: rows[i].note,
                    budget_category_id: category ? Number(category) : null,
                })),
            });
            onDone();
        } catch (problem: any) {
            ...
            {rows && rows.length > 0 && (
                <>
                    ...
                    <div className="max-h-80 overflow-y-auto rounded-lg border border-[var(--color-border)]">
                        {rows.map((radek, i) => (
                            <label key={i}
                                className={`flex cursor-pointer items-center gap-2.5 border-b border-[var(--color-border)] px-2.5 py-1.5 last:border-b-0 ${chosen.has(i) ? '' : 'opacity-45'}`}>
                                ...
                                <span className={`shrink-0 text-xs ${radek.kind === 'income' ? 'text-emerald-300' : 'text-[var(--color-text-primary)]'}`}>
