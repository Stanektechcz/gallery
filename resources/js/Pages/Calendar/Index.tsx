...
export default function CalendarIndex() {
    ...
    return <AppLayout><Head title="Kalendář a plánování" />
        <main className="mx-auto max-w-6xl p-4 sm:p-6">
            ...
            <div className="mb-4 flex flex-wrap justify-end gap-2"><button onClick={() => { setShowSharedSlots(value => !value); if (!showSharedSlots) loadSharedSlots(); }} className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-sky-400/30 bg-sky-500/10 px-3 text-sm text-sky-100 hover:bg-sky-500/20"><Bell size={15}/> Společný termín</button><button onClick={() => { setShowIdeas(value => !value); if (!showIdeas) loadIdeas(); }} className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-orange-400/30 bg-orange-500/10 px-3 text-sm text-orange-100 hover:bg-orange-500/20"><MapPin size={15}/> Tip z uložených míst</button><Link href="/planning#date-generator" className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-pink-400/30 bg-pink-500/10 px-3 text-sm text-pink-100 hover:bg-pink-500/20"><Sparkles size={15}/> Generátor randíček</Link></div>
