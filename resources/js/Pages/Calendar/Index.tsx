...
export default function CalendarIndex({ today }: { today: string }) {
    ...
    return <AppLayout><Head title="Kalendář a plánování" />
        <main className="w-full p-3 sm:p-6 lg:p-8">
            ...
            </div><div className="mt-5 flex justify-end gap-2"><button type="button" onClick={() => setShowCreate(false)} className="rounded-lg px-3 py-2 text-sm text-[var(--color-text-secondary)]">Zrušit</button><button disabled={saving} className="rounded-lg bg-[var(--color-accent)] px-4 py-2 text-sm font-medium text-[var(--color-text-primary)] disabled:opacity-50">{saving ? 'Ukládám…' : 'Vytvořit akci'}</button></div></form></div>}
            {showImport && <div className="fixed inset-0 z-50 flex items-end bg-black/60 p-0 sm:items-center sm:justify-center sm:p-4">
                <form onSubmit={importIcs} className="max-h-[94vh] w-full overflow-y-auto rounded-t-2xl border border-[var(--color-border)] bg-[var(--color-bg-secondary)] p-5 sm:max-w-xl sm:rounded-2xl">
                    ...
                    <label className="mt-3 flex min-h-20 cursor-pointer flex-col items-center justify-center rounded-xl border border-dashed border-[var(--color-accent)]/50 bg-[var(--color-accent)]/5 p-3 text-center text-sm text-[var(--color-text-primary)] hover:bg-[var(--color-accent)]/10"><Upload size={18}/><span className="mt-1">{icsFileName || 'Vybrat soubor .ics'}</span><span className="text-[10px] text-[var(--color-text-secondary)]">maximálně 512 KB</span><input type="file" accept=".ics,text/calendar" onChange={loadIcsFile} className="sr-only"/></label>
                    ...
                    <div className="mt-4 flex justify-end gap-2"><button type="button" onClick={() => setShowImport(false)} className="rounded-lg px-3 py-2 text-sm text-[var(--color-text-secondary)]">Zrušit</button><button disabled={importing || !ics.trim()} className="rounded-lg bg-[var(--color-accent)] px-4 py-2 text-sm font-medium text-[var(--color-text-primary)] disabled:opacity-50">{importing ? 'Importuji…' : 'Importovat a propojit'}</button></div>
                </form>
            </div>}
            <CalendarEventEditor
