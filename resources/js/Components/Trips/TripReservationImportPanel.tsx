    ...
    return <section className="mb-5 overflow-hidden rounded-3xl border border-violet-400/25 bg-gradient-to-br from-violet-500/10 via-[var(--color-bg-card)] to-sky-500/5">
        ...
        {open&&<div className="border-t border-violet-300/15 p-4 sm:p-5">
            <form onSubmit={upload} className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto]">
                <label className="flex min-h-20 cursor-pointer items-center gap-3 rounded-2xl border border-dashed border-violet-300/30 bg-[var(--color-surface-muted)] px-3"><Upload size={17} className="shrink-0 text-violet-200"/><span className="min-w-0"><span className="block truncate text-xs text-[var(--color-text-primary)]">{file?.name??'PDF, obrázek, e-mail nebo text'}</span><span className="mt-1 block text-[9px] text-[var(--color-text-secondary)]">Max. 20 MB · soubor zůstává neveřejný</span></span><input ref={inputRef} type="file" accept=".pdf,.jpg,.jpeg,.png,.webp,.txt,.eml,.csv,.json" onChange={event=>setFile(event.target.files?.[0]??null)} className="hidden"/></label>
                <textarea value={sourceText} onChange={event=>setSourceText(event.target.value)} placeholder="Nebo vložte text potvrzovacího e-mailu…" className="min-h-20 resize-y rounded-2xl border border-violet-300/20 bg-[var(--color-surface-muted)] px-3 py-2 text-xs text-[var(--color-text-primary)] placeholder:text-[var(--color-text-secondary)]"/>
            ...
            {(pending.length>0||confirmed.length>0)&&<div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{[...pending,...confirmed].map(item=>{const data=item.confirmed_data??item.extracted_data;return <div key={item.uuid} className={`rounded-2xl border p-3 ${item.status==='confirmed'?'border-emerald-400/20 bg-emerald-500/5':'border-amber-400/25 bg-amber-500/5'}`}><div className="flex items-start gap-2"><span className="min-w-0 flex-1"><span className="block truncate text-xs font-medium text-[var(--color-text-primary)]">{data.title||item.original_name||'Cestovní podklad'}</span><span className="mt-1 block truncate text-[9px] text-[var(--color-text-secondary)]">{TYPE_LABELS[data.type]??'Rezervace'}{data.reference?` · ${data.reference}`:''}</span></span>{item.status==='confirmed'?<CheckCircle2 size={15} className="shrink-0 text-emerald-300"/>:<span className="shrink-0 rounded-full bg-amber-400/10 px-1.5 py-0.5 text-[8px] text-amber-100">kontrola</span>}</div><div className="mt-3 flex gap-1"><button type="button" onClick={()=>beginReview(item)} className="min-h-8 flex-1 rounded-lg border border-[var(--color-border)] text-[10px] text-[var(--color-text-primary)]">{item.status==='confirmed'?'Zkontrolovat údaje':'Otevřít kontrolu'}</button>{item.has_file&&<a href={`/api/v1/trips/${tripId}/reservation-imports/${item.uuid}/download`} className="flex min-h-8 items-center justify-center rounded-lg border border-[var(--color-border)] px-2 text-[var(--color-text-secondary)]" title="Původní soubor"><Paperclip size={12}/></a>}{item.status!=='confirmed'&&<button type="button" disabled={busy===item.uuid} onClick={()=>remove(item)} className="flex min-h-8 items-center justify-center rounded-lg border border-red-400/15 px-2 text-red-300"><Trash2 size={12}/></button>}</div></div>})}</div>}

            {selected&&draft&&<form onSubmit={confirmImport} className="mt-5 rounded-3xl border border-violet-300/25 bg-[var(--color-surface-muted)] p-4"><div className="flex flex-wrap items-start justify-between gap-2"><div><p className="text-sm font-semibold text-[var(--color-text-primary)]">Kontrola před propojením</p><p className="mt-1 text-[10px] text-[var(--color-text-secondary)]">Rozpoznané hodnoty jsou návrh. Vše lze před uložením opravit.</p></div>{selected.extracted_data?.confidence!=null&&<span className="rounded-full bg-[var(--color-surface-muted)] px-2 py-1 text-[9px] text-violet-100">jistota {Math.round(Number(selected.extracted_data.confidence)*100)} %</span>}</div>
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                    <label className="text-[9px] text-[var(--color-text-secondary)]">Typ<select value={draft.type} onChange={event=>setDraft({...draft,type:event.target.value})} className="mt-1 min-h-10 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-secondary)] px-2 text-xs text-[var(--color-text-primary)]">{Object.entries(TYPE_LABELS).map(([key,label])=><option key={key} value={key}>{label}</option>)}</select></label>
                    <label className="text-[9px] text-[var(--color-text-secondary)] sm:col-span-2">Název<input required value={draft.title} onChange={event=>setDraft({...draft,title:event.target.value})} className="mt-1 min-h-10 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-muted)] px-3 text-xs text-[var(--color-text-primary)]"/></label>
                    <label className="text-[9px] text-[var(--color-text-secondary)]">Poskytovatel<input value={draft.provider??''} onChange={event=>setDraft({...draft,provider:event.target.value})} placeholder="RegioJet, Booking…" className="mt-1 min-h-10 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-muted)] px-3 text-xs text-[var(--color-text-primary)]"/></label>
                    <label className="text-[9px] text-[var(--color-text-secondary)]">Kód rezervace<input value={draft.reference??''} onChange={event=>setDraft({...draft,reference:event.target.value})} className="mt-1 min-h-10 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-muted)] px-3 text-xs text-[var(--color-text-primary)]"/></label>
                    <label className="text-[9px] text-[var(--color-text-secondary)]">Začátek<input type="datetime-local" value={draft.starts_at??''} onChange={event=>setDraft({...draft,starts_at:event.target.value})} className="mt-1 min-h-10 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-muted)] px-2 text-xs text-[var(--color-text-primary)]"/></label>
                    <label className="text-[9px] text-[var(--color-text-secondary)]">Konec<input type="datetime-local" value={draft.ends_at??''} onChange={event=>setDraft({...draft,ends_at:event.target.value})} className="mt-1 min-h-10 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-muted)] px-2 text-xs text-[var(--color-text-primary)]"/></label>
                    <label className="text-[9px] text-[var(--color-text-secondary)]">Den itineráře<select value={draft.trip_day_id??''} onChange={event=>setDraft({...draft,trip_day_id:event.target.value})} className="mt-1 min-h-10 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-secondary)] px-2 text-xs text-[var(--color-text-primary)]"><option value="">Bez dne</option>{days.map(day=><option key={day.id} value={day.id}>{new Date(`${day.date}T12:00:00`).toLocaleDateString('cs-CZ',{day:'numeric',month:'short'})}{day.title?` · ${day.title}`:''}</option>)}</select></label>
                    <label className="text-[9px] text-[var(--color-text-secondary)]">Odkud<input value={draft.origin??''} onChange={event=>setDraft({...draft,origin:event.target.value})} className="mt-1 min-h-10 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-muted)] px-3 text-xs text-[var(--color-text-primary)]"/></label>
                    <label className="text-[9px] text-[var(--color-text-secondary)]">Kam<input value={draft.destination??''} onChange={event=>setDraft({...draft,destination:event.target.value})} className="mt-1 min-h-10 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-muted)] px-3 text-xs text-[var(--color-text-primary)]"/></label>
                    <label className="text-[9px] text-[var(--color-text-secondary)]">Místo / adresa<input value={draft.place_name??''} onChange={event=>setDraft({...draft,place_name:event.target.value})} className="mt-1 min-h-10 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-muted)] px-3 text-xs text-[var(--color-text-primary)]"/></label>
                    <label className="text-[9px] text-[var(--color-text-secondary)]">Cena<div className="mt-1 flex"><input type="number" min="0" step="0.01" value={draft.amount??''} onChange={event=>setDraft({...draft,amount:event.target.value})} className="min-h-10 min-w-0 flex-1 rounded-l-xl border border-[var(--color-border)] bg-[var(--color-surface-muted)] px-2 text-xs text-[var(--color-text-primary)]"/><input maxLength={3} value={draft.currency??currency} onChange={event=>setDraft({...draft,currency:event.target.value.toUpperCase()})} className="min-h-10 w-14 rounded-r-xl border border-l-0 border-[var(--color-border)] bg-[var(--color-surface-muted)] px-2 text-xs text-[var(--color-text-primary)]"/></div></label>
                    <label className="text-[9px] text-[var(--color-text-secondary)] sm:col-span-2 lg:col-span-3">Poznámka<textarea value={draft.notes??''} onChange={event=>setDraft({...draft,notes:event.target.value})} className="mt-1 min-h-20 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-muted)] px-3 py-2 text-xs text-[var(--color-text-primary)]"/></label>
                    <label className="text-[9px] text-[var(--color-text-secondary)]">Připomenout předem (hodiny)<input value={draft.reminder_hours} onChange={event=>setDraft({...draft,reminder_hours:event.target.value})} placeholder="24,2" className="mt-1 min-h-10 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-muted)] px-3 text-xs text-[var(--color-text-primary)]"/></label>
                </div>
                <div className="mt-4 grid gap-2 sm:grid-cols-2"><label className="flex items-start gap-2 rounded-xl border border-[var(--color-border)] p-3 text-[10px] text-violet-100"><input type="checkbox" checked={draft.sync_itinerary} onChange={event=>setDraft({...draft,sync_itinerary:event.target.checked})}/><Route size={14} className="shrink-0"/><span><strong className="block text-[var(--color-text-primary)]">Přidat do itineráře</strong>Čas, místo a cena se stanou upravitelným blokem dne.</span></label><label className="flex items-start gap-2 rounded-xl border border-[var(--color-border)] p-3 text-[10px] text-violet-100"><input type="checkbox" checked={draft.sync_calendar} onChange={event=>setDraft({...draft,sync_calendar:event.target.checked})}/><CalendarPlus size={14} className="shrink-0"/><span><strong className="block text-[var(--color-text-primary)]">Kalendář a upozornění</strong>Oba partneři uvidí termín a dostanou zvolená připomenutí.</span></label></div>
                <div className="mt-4 flex flex-col gap-2 sm:flex-row"><button disabled={busy===selected.uuid} className="min-h-11 flex-1 rounded-xl bg-violet-500 px-4 text-xs font-medium text-white disabled:opacity-40">{busy===selected.uuid?'Propojuji…':selected.status==='confirmed'?'Aktualizovat propojené údaje':'Potvrdit a propojit napříč systémem'}</button><button type="button" onClick={()=>{setSelected(null);setDraft(null);}} className="min-h-11 rounded-xl border border-[var(--color-border)] px-4 text-xs text-[var(--color-text-secondary)]">Zavřít</button></div>
                {selected.source_excerpt&&<details className="mt-3"><summary className="cursor-pointer text-[9px] text-violet-200/70">Ukázat výřez rozpoznaného textu</summary><p className="mt-2 whitespace-pre-wrap rounded-xl bg-[var(--color-surface-muted)] p-3 text-[9px] leading-relaxed text-[var(--color-text-secondary)]">{selected.source_excerpt}</p></details>}
            </form>}
