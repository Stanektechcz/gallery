...
function SuggestionCard({ suggestion, gallerySpaceId, onRemove }: { suggestion: AlbumSuggestion; gallerySpaceId: number; onRemove: (fingerprint: string) => void }) {
    ...
    return (
        <article className="overflow-hidden rounded-2xl border border-violet-400/20 bg-[var(--color-surface-muted)]">
            <div className="grid grid-cols-5 gap-1 bg-[var(--color-bg-secondary)] p-1">
                {suggestion.preview.map((item, index) => <div key={item.uuid} className={`relative overflow-hidden rounded-lg bg-[var(--color-surface-muted)] ${index === 0 ? 'col-span-2 row-span-2 aspect-square' : 'aspect-square'}`}>
                    {item.thumbnail_url ? <img src={item.thumbnail_url} alt="" loading="lazy" decoding="async" className="h-full w-full object-cover"/> : <div className="flex h-full items-center justify-center text-[var(--color-text-primary)]/25"><Images size={18}/></div>}
                    {item.media_type === 'video' && <Video size={13} className="absolute bottom-1 right-1 text-[var(--color-text-primary)] drop-shadow"/>}
                </div>)}
            </div>
            <div className="p-4">
                <div className="flex items-start justify-between gap-3"><div className="min-w-0"><h3 className="truncate font-semibold text-[var(--color-text-primary)]">{suggestion.title}</h3><p className="mt-1 text-xs text-violet-100">{suggestion.reason}</p></div><span className="shrink-0 rounded-full bg-violet-500/10 px-2 py-1 text-[10px] text-violet-100">{suggestion.target_album ? 'doplnit album' : 'nové album'}</span></div>
                ...
                {expanded && <div className="mt-3 space-y-3 border-t border-[var(--color-border)] pt-3">
                    <label className="block"><span className="text-xs text-[var(--color-text-secondary)]">Název alba</span><input value={title} onChange={e => setTitle(e.target.value)} maxLength={255} className="mt-1 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-muted)] px-3 py-2 text-sm text-[var(--color-text-primary)]"/></label>
                    <label className="block"><span className="text-xs text-[var(--color-text-secondary)]">Popis společné vzpomínky</span><textarea value={description} onChange={e => setDescription(e.target.value)} rows={2} maxLength={5000} className="mt-1 w-full resize-y rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-muted)] px-3 py-2 text-sm text-[var(--color-text-primary)]"/></label>
                    <div><div className="mb-2 flex items-center justify-between text-xs"><span className="text-[var(--color-text-secondary)]">Vybráno {selected.size} z {suggestion.media.length}</span><span className="text-violet-200">Klepnutím vyřadíte</span></div><div className="grid max-h-64 grid-cols-4 gap-2 overflow-y-auto pr-1 sm:grid-cols-6 lg:grid-cols-8">
                        {suggestion.media.map(item => <button type="button" key={item.uuid} onClick={() => toggle(item.uuid)} className={`group relative aspect-square overflow-hidden rounded-lg border-2 ${selected.has(item.uuid) ? 'border-violet-400' : 'border-transparent opacity-40'}`} title={item.title}>
                            {item.thumbnail_url ? <img src={item.thumbnail_url} alt="" loading="lazy" decoding="async" className="h-full w-full object-cover"/> : <div className="flex h-full items-center justify-center bg-[var(--color-surface-muted)] text-[var(--color-text-primary)]/30"><Images size={16}/></div>}
