        ...
        <div className="fixed bottom-[4.5rem] right-4 z-[700] md:bottom-5 md:right-5">
            {open && <section aria-label="Maki pomocník" className="mb-3 flex h-[min(640px,74dvh)] w-[min(410px,calc(100vw-2rem))] flex-col overflow-hidden rounded-3xl border border-violet-400/30 bg-[var(--color-bg-secondary)] shadow-2xl shadow-black/50">
                <header className="flex items-center gap-3 border-b border-[var(--color-border)] bg-gradient-to-r from-violet-500/15 to-pink-500/10 p-4">
                    ...
                    <div className="min-w-0 flex-1"><h2 className="font-semibold text-white">Maki pomocník</h2><p className="text-xs text-violet-100">Lokální asistent · vždy s potvrzením</p></div>
                    <button type="button" onClick={clearChat} aria-label="Vyčistit historii chatu" title="Vyčistit historii chatu" className="rounded-xl p-2 text-[var(--color-text-secondary)] hover:bg-white/10 hover:text-white"><Trash2 size={16} /></button><button onClick={() => setOpen(false)} aria-label="Zavřít chat" className="rounded-xl p-2 text-[var(--color-text-secondary)] hover:bg-white/10 hover:text-white"><X size={18} /></button>
                </header>
                ...
                <div className="flex-1 space-y-3 overflow-y-auto p-3">
                    {messages.map((message, index) => {
                        ...
                        return <article key={index} className={`max-w-[94%] rounded-2xl p-3 text-sm ${message.role === 'user' ? 'ml-auto bg-violet-600 text-white' : 'bg-black/15 text-[var(--color-text-primary)]'}`}>
                        <p>{message.text}</p>
                    ...
                    {loading && <div className="inline-flex items-center gap-2 rounded-2xl bg-black/15 p-3 text-xs text-[var(--color-text-secondary)]"><LoaderCircle size={14} className="animate-spin" /> Připravuji náhled…</div>}
                    <div ref={end} />
                </div>
                <form onSubmit={preview} className="border-t border-[var(--color-border)] p-3">
                    {showAttachments && <div className="mb-2 rounded-xl border border-violet-400/25 bg-black/10 p-2"><UploadZone albumId={null} onUploadComplete={(uuids) => setAttachmentUuids(current => Array.from(new Set([...current, ...uuids])))} /><p className="mt-2 text-[10px] text-violet-100">Nahrané fotky se po potvrzení připojí k albu dané události. Vybráno: {attachmentUuids.length}.</p></div>}
                    <div className="flex gap-2"><textarea value={input} onChange={(event) => setInput(event.target.value)} rows={2} placeholder="Popište, co se stalo nebo naplánujte…" className="min-h-11 flex-1 resize-none rounded-xl border border-[var(--color-border)] bg-black/15 px-3 py-2 text-sm text-white placeholder:text-[var(--color-text-secondary)]" /><button type="button" onClick={() => setShowAttachments(value => !value)} aria-label="Přiložit fotografie" title="Přiložit fotografie" className={`grid h-11 w-11 place-items-center rounded-xl border ${showAttachments || attachmentUuids.length ? 'border-violet-300 bg-violet-500/20 text-violet-100' : 'border-[var(--color-border)] text-[var(--color-text-secondary)]'}`}><ImagePlus size={16}/></button><button disabled={(!input.trim() && !attachmentUuids.length) || loading} aria-label="Odeslat" className="grid h-11 w-11 place-items-center rounded-xl bg-violet-500 text-white disabled:opacity-40"><Send size={16} /></button></div>
                    {matchingCommands.length > 0 && <div className="mt-2 overflow-hidden rounded-xl border border-violet-400/25 bg-black/20"><div className="flex items-center justify-between border-b border-white/5 px-3 py-1.5 text-[10px] text-[var(--color-text-secondary)]"><span>Katalog příkazů</span>{recentCommands.length > 0 && <span>Nejčastější nahoře</span>}</div>{matchingCommands.map((hint) => <button key={hint.command} type="button" onClick={() => chooseCommand(hint)} className="flex w-full gap-3 border-b border-white/5 px-3 py-2 text-left text-xs last:border-0 hover:bg-violet-500/10"><code className="shrink-0 font-semibold text-violet-200">{hint.command}</code><span className="min-w-0"><span className="block text-[var(--color-text-primary)]">{hint.description}</span><span className="mt-0.5 block truncate text-[10px] text-[var(--color-text-secondary)]">{hint.example}</span></span></button>)}</div>}
