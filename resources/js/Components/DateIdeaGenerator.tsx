...
export default function DateIdeaGenerator({ spaceId }:{ spaceId?:number }) {
    ...
    return <section id="date-generator" className="mt-5 overflow-hidden rounded-3xl border border-pink-400/25 bg-gradient-to-br from-pink-500/10 via-[var(--color-bg-card)] to-violet-500/10">
        <div className="p-4 sm:p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-medium uppercase tracking-wider text-pink-200">Generátor společných randíček</p><h2 className="mt-1 flex items-center gap-2 text-lg font-semibold text-[var(--color-text-primary)]"><Sparkles size={19} className="text-pink-300"/>Ne náhodný text, ale plán pro vás dva</h2><p className="mt-1 max-w-3xl text-sm text-[var(--color-text-secondary)]">Kombinuje místo, rozpočet, dopravu, volný kalendář, počasí, uložené podniky i vaše reakce. Stejnou kombinaci znovu nenabídne.</p></div><Link href="/calendar" className="panel-link shrink-0 text-sm text-pink-200 hover:text-[var(--color-text-primary)]">Společný kalendář →</Link></div>

            <div className="mt-5"><p className="mb-2 text-xs font-medium text-[var(--color-text-secondary)]">Na co máte chuť?</p><div className="flex snap-x gap-2 overflow-x-auto pb-2">{THEMES.map(([value,label])=><button key={value} type="button" onClick={()=>setForm({...form,theme:value})} className={`min-h-10 shrink-0 snap-start rounded-full border px-3 text-sm ${form.theme===value?'border-pink-300 bg-pink-500/20 text-pink-100':'border-[var(--color-border)] text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)]'}`}>{label}</button>)}</div></div>
            ...
            <details className="mt-4 rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface-muted)] p-3"><summary className="panel-link cursor-pointer text-sm font-medium text-[var(--color-text-primary)]">Doladit atmosféru, dopravu a omezení</summary><div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <label className="flex min-h-11 items-center gap-2 rounded-xl border border-[var(--color-border)] px-3 text-xs text-[var(--color-text-primary)]"><input type="checkbox" checked={form.weather_aware} onChange={event=>setForm({...form,weather_aware:event.target.checked})}/> Přizpůsobit předpovědi</label>
                <label className="flex min-h-11 items-center gap-2 rounded-xl border border-[var(--color-border)] px-3 text-xs text-[var(--color-text-primary)]"><input type="checkbox" checked={form.accessible_only} onChange={event=>setForm({...form,accessible_only:event.target.checked})}/> Jen bezbariérová místa</label>
                <label className="flex min-h-11 items-center gap-2 rounded-xl border border-[var(--color-border)] px-3 text-xs text-[var(--color-text-primary)]"><input type="checkbox" checked={form.new_places_only} onChange={event=>setForm({...form,new_places_only:event.target.checked})}/> Jen dosud nehodnocená místa</label>
            </div></details>

            <button type="button" onClick={generate} disabled={loading||!spaceId} className="mt-4 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-pink-600 to-violet-600 px-5 text-sm font-semibold text-white shadow-lg shadow-pink-950/20 disabled:opacity-50 sm:w-auto">{loading?<RefreshCw size={17} className="animate-spin"/>:<Sparkles size={17}/>} {loading?'Skládám nové neopakované varianty…':'Vygenerovat 4 unikátní randíčka'}</button>
        ...
        <div className="border-t border-pink-400/15 bg-[var(--color-surface-muted)] p-4 sm:p-5"><div className="flex items-center justify-between"><div><h3 className="font-semibold text-[var(--color-text-primary)]">Návrhy a společná historie</h3><p className="mt-1 text-xs text-[var(--color-text-secondary)]">Srdce ovlivní další generování pro oba; přeskočené varianty se schovají.</p></div>{initialLoading&&<RefreshCw size={16} className="animate-spin text-pink-200"/>}</div>
            ...
            {!initialLoading&&!shown.length&&<div className="mt-4 rounded-2xl border border-dashed border-pink-300/25 p-6 text-center"><Sparkles className="mx-auto text-pink-300"/><p className="mt-2 text-sm text-[var(--color-text-primary)]">Nastavte náladu a nechte si sestavit první čtyři varianty.</p><p className="mt-1 text-xs text-[var(--color-text-secondary)]">Můžete začít i bez cíle; generátor použije domácí a obecné aktivity.</p></div>}
        </div>
    </section>;
}
...
const inputClass='mt-1 min-h-11 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-secondary)] px-3 text-sm text-[var(--color-text-primary)] focus:border-pink-400 focus:outline-none';
