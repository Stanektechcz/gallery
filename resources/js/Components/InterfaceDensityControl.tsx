...
export default function InterfaceDensityControl({ initial }: { initial?: unknown }) {
    ...
    return <div className="relative">
        <button type="button" aria-label="Hustota rozhraní" aria-expanded={open} onClick={() => setOpen(value => !value)} className="flex h-9 w-9 items-center justify-center rounded-lg text-[var(--color-text-secondary)] hover:bg-white/5 hover:text-white"><SlidersHorizontal size={16}/></button>
        {open && <><button type="button" aria-label="Zavřít nabídku hustoty" onClick={() => setOpen(false)} className="fixed inset-0 z-40 cursor-default"/><div className="absolute bottom-full right-0 z-50 mb-2 w-64 rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-card)] p-2 shadow-2xl"><p className="px-2 py-1 text-xs font-medium text-white">Hustota rozhraní</p>{options.map(option => <button key={option.value} type="button" disabled={busy} onClick={() => void choose(option.value)} className={`mt-1 flex w-full items-center gap-2 rounded-lg px-2 py-2 text-left disabled:opacity-50 ${density === option.value ? 'bg-[var(--color-accent)]/15 text-white' : 'text-[var(--color-text-secondary)] hover:bg-white/5 hover:text-white'}`}><span className="flex h-5 w-5 items-center justify-center">{density === option.value && <Check size={14}/>}</span><span><span className="block text-xs">{option.label}</span><span className="block text-[10px] opacity-75">{option.note}</span></span></button>)}</div></>}
    </div>;
}