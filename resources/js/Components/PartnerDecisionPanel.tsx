...
const tone:Record<string,string>={
    ...
    negative:'border-[var(--color-border)] text-[var(--color-text-secondary)] hover:border-red-300/25 hover:text-red-100',
...
export default function PartnerDecisionPanel({spaceId,initialData=null,compact=false}:{spaceId?:number|null;initialData?:PartnerDecisionSnapshot|null;compact?:boolean}){
    ...
    return <section id="partner-decisions" className="scroll-mt-24 rounded-3xl border border-violet-400/25 bg-gradient-to-br from-violet-500/10 via-[var(--color-bg-card)] to-pink-500/5 p-4 sm:p-5">
        ...
        {!compact&&<div className="mt-3 flex flex-wrap gap-x-4 gap-y-2 text-[11px]"><Link href="/date-ideas" className="panel-link text-pink-200">Generovat randíčko →</Link><Link href="/watchlist" className="panel-link text-violet-200">Přidat film nebo seriál →</Link><Link href="/planning" className="panel-link text-teal-200">Vytvořit anketu →</Link></div>}
    </section>;
}
