...
export default function RelationshipAnniversarySettings({ spaceId, showRecap = true }: { spaceId?: number; showRecap?: boolean }) {
    ...
    return <section id="relationship-anniversary" className="scroll-mt-20 lg:col-span-2 rounded-2xl border border-pink-500/25 bg-gradient-to-br from-pink-500/10 to-[var(--color-bg-card)] p-4">
        <h2 className="flex items-center gap-2 font-semibold text-[var(--color-text-primary)]"><CalendarHeart size={17} className="text-pink-300"/>Naše výročí</h2>
        ...
        <form onSubmit={save} className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-end">
            <label className="flex-1 text-xs text-pink-100">Začátek vztahu
                <input required max={new Date().toISOString().slice(0, 10)} type="date" value={startedOn} onChange={item => setStartedOn(item.target.value)} className="mt-1 block min-h-10 w-full rounded-lg border border-pink-300/25 bg-[var(--color-surface-muted)] px-3 text-sm text-[var(--color-text-primary)]"/>
            </label>
        ...
        {duration && <div className="mt-4 grid grid-cols-3 gap-2 rounded-xl border border-pink-300/20 bg-[var(--color-surface-muted)] p-3 text-center">
            <div><p className="text-lg font-semibold text-[var(--color-text-primary)]">{number.format(duration.years)}</p><p className="text-[10px] uppercase tracking-wide text-pink-200">let spolu</p></div>
            <div><p className="text-lg font-semibold text-[var(--color-text-primary)]">{number.format(duration.days)}</p><p className="text-[10px] uppercase tracking-wide text-pink-200">dní spolu</p></div>
            <div><p className="text-lg font-semibold text-[var(--color-text-primary)]">{number.format(duration.hours)}</p><p className="text-[10px] uppercase tracking-wide text-pink-200"><Timer className="mr-1 inline" size={11}/>hodin spolu</p></div>
        </div>}
        {notice && <p className="mt-3 text-sm text-emerald-200">{notice}</p>}
        {error && <p className="mt-3 text-sm text-red-200">{error}</p>}
        {events.length > 0 && <div className="mt-3 flex flex-wrap gap-2">{events.map(item => <span key={item.uuid} className="rounded-lg border border-pink-300/20 bg-[var(--color-surface-muted)] px-3 py-2 text-xs text-pink-100">❤️ {item.title} · {new Date(item.starts_at).toLocaleDateString('cs-CZ')}{item.recurrence_rule?.frequency === 'yearly' ? ' · každý rok' : ''}</span>)}</div>}
