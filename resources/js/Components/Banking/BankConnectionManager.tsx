...
export default function BankConnectionManager({ gallerySpaceId, returnTripId, compact = false, onChanged }: {
    ...
    const importStatement=async(file:File)=>{
        ...
        try{
            ...
            setMessage(response.data.duplicate_file?'Tento výpis už byl uložen; žádná data se nezdvojila.':`Hotovo: ${response.data.import.rows_imported} nových transakcí, ${response.data.import.rows_duplicate} duplicit přeskočeno${response.data.import.rows_failed?`, ${response.data.import.rows_failed} řádků vyžaduje kontrolu`:''}.`);
        }catch(reason:any){setError(reason.response?.data?.message??'Výpis se nepodařilo importovat.');}
        finally{setBusy('');}
    };
    const upload=async(event:ChangeEvent<HTMLInputElement>)=>{
        const file=event.target.files?.[0]; if(file)await importStatement(file); event.target.value='';
    };
    const drop=async(event:DragEvent<HTMLLabelElement>)=>{
        event.preventDefault(); const file=event.dataTransfer.files?.[0]; if(file&&!busy)await importStatement(file);
    };
    ...
    return <section className={`rounded-3xl border border-emerald-400/20 bg-gradient-to-br from-emerald-500/10 via-[var(--color-bg-card)] to-[var(--color-bg-card)] ${compact?'p-3 sm:p-4':'p-4 sm:p-5'}`}>
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-emerald-200"><WalletCards size={15}/> Společný účet a cesty</p><h2 className="mt-1 text-lg font-semibold text-white">Historie společného Revolutu</h2><p className="mt-1 max-w-2xl text-xs leading-relaxed text-[var(--color-text-secondary)]">Nahrajte export z Revolutu a aplikace bezpečně uloží transakce, zůstatky, grafy i vazby na cesty. Překrývající se výpisy automaticky spojí bez zdvojení plateb.</p></div><span className="inline-flex w-fit items-center gap-1 rounded-full border border-emerald-300/20 bg-emerald-500/10 px-2.5 py-1 text-[10px] text-emerald-100"><LockKeyhole size={11}/> data zůstávají v galerii</span></div>
        {error&&<p className="mt-3 rounded-xl border border-red-400/20 bg-red-500/10 p-3 text-xs leading-relaxed text-red-100">{error}</p>}{message&&<p className="mt-3 rounded-xl border border-emerald-400/20 bg-emerald-500/10 p-3 text-xs text-emerald-100">{message}</p>}
        ...
        {active.length===0&&<details className="mt-4 rounded-2xl border border-white/10 bg-black/5 p-3"><summary className="cursor-pointer text-xs font-medium text-[var(--color-text-secondary)]">Volitelné automatické připojení přes GoCardless</summary><div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-end"><div className="flex-1"><p className="text-[10px] leading-relaxed text-[var(--color-text-secondary)]">Pokud bude automatické bankovní napojení znovu dostupné, lze jej aktivovat zde. Pro běžné používání stačí ruční import níže.</p>{institutions.length>0&&<select value={institutionId} onChange={event=>setInstitutionId(event.target.value)} className="mt-2 min-h-10 w-full rounded-lg border border-white/10 bg-[var(--color-bg-primary)] px-3 text-xs text-white">{institutions.map(item=><option key={item.id} value={item.id}>{item.name}{item.transaction_total_days?` · historie až ${item.transaction_total_days} dní`:''}</option>)}</select>}</div>{institutions.length===0?<button onClick={findInstitutions} disabled={!!busy} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-xl border border-emerald-300/25 px-4 text-xs text-emerald-100 disabled:opacity-50"><Building2 size={14}/>{busy==='institutions'?'Načítám…':'Vyhledat připojení'}</button>:<button onClick={connect} disabled={!!busy||!institutionId} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-xl border border-emerald-300/25 px-4 text-xs text-emerald-100 disabled:opacity-50">Přejít k připojení <ExternalLink size={13}/></button>}</div></details>}
        <label onDragOver={event=>event.preventDefault()} onDrop={drop} className={`mt-4 flex cursor-pointer flex-col items-center justify-center rounded-2xl border border-dashed p-5 text-center transition ${busy==='import'?'cursor-wait border-emerald-300/40 bg-emerald-500/10':'border-emerald-300/25 bg-black/10 hover:border-emerald-300/50 hover:bg-emerald-500/5'}`}><span className="grid h-11 w-11 place-items-center rounded-2xl bg-emerald-500/15 text-emerald-200">{busy==='import'?<RefreshCw size={20} className="animate-spin"/>:<FileSpreadsheet size={20}/>}</span><p className="mt-3 text-sm font-medium text-white">{busy==='import'?'Načítám a propojuji transakce…':'Přetáhněte sem výpis nebo jej vyberte'}</p><p className="mt-1 max-w-xl text-[10px] leading-relaxed text-[var(--color-text-secondary)]">CSV, XLS i XLSX · české i anglické názvy sloupců · Amount/Částka i samostatné Paid out/Paid in, Výdaj/Příjem nebo Debit/Credit · až 50 MB</p><span className="mt-3 inline-flex min-h-9 items-center gap-2 rounded-xl border border-emerald-300/25 px-4 text-xs text-emerald-100"><Upload size={14}/>Vybrat export z Revolutu</span><input type="file" accept=".csv,.xls,.xlsx,text/csv,application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" disabled={!!busy} onChange={upload} className="hidden"/></label>
