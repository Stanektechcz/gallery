...
export default function BankConnectionManager({ gallerySpaceId, returnTripId, compact = false, onChanged }: {
    ...
    return <section className={`rounded-3xl border border-emerald-400/20 bg-gradient-to-br from-emerald-500/10 via-[var(--color-bg-card)] to-[var(--color-bg-card)] ${compact?'p-3 sm:p-4':'p-4 sm:p-5'}`}>
        ...
        {error&&<p className="mt-3 rounded-xl border border-red-400/20 bg-red-500/10 p-3 text-xs text-red-100">{error} <Link href="/admin/integrations#gocardless_bank_data" className="font-medium underline">Otevřít konfiguraci GoCardless</Link></p>}{message&&<p className="mt-3 rounded-xl border border-emerald-400/20 bg-emerald-500/10 p-3 text-xs text-emerald-100">{message}</p>}
