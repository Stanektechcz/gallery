...
export default function GiftAutomation({ spaceId }: { spaceId?: number }) {
    ...
    return <section className="rounded-2xl border border-amber-400/20 bg-[var(--color-bg-card)] p-4 sm:p-5">
        ...
        <div className="mt-4 grid gap-2 md:grid-cols-2">{gifts.map(gift => { const currentStage = gift.lifecycle?.at(-1)?.stage ?? (gift.status === 'planned' ? 'planned' : gift.status === 'purchased' ? 'purchased' : 'idea'); return <div key={gift.uuid} className="rounded-xl border border-[var(--color-border)] bg-black/10 p-3 text-sm"><div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center"><div className="min-w-0"><p className="flex items-center gap-1.5 truncate text-white">{gift.is_private && <Lock size={13} className="shrink-0 text-amber-200"/>}<span className="truncate">{gift.title}</span></p><p className="text-xs text-[var(--color-text-secondary)]">{gift.due_date ? new Date(`${gift.due_date}T12:00:00`).toLocaleDateString('cs-CZ') : 'bez data'} · {GIFT_STAGES.find(stage => stage.key === currentStage)?.label}</p></div><select aria-label={`Kdo zařídí ${gift.title}`} value={gift.assigned_to ?? ''} onChange={event => assignGift(gift.uuid, event.target.value)} className="min-h-9 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-secondary)] px-2 text-xs text-white"><option value="">Kdo zařídí?</option>{members.map(member => <option key={member.id} value={member.id}>{member.name}</option>)}</select></div><div className="mt-3 flex flex-wrap gap-1.5" aria-label={`Časová osa dárku ${gift.title}`}>{GIFT_STAGES.map(stage => { const done = GIFT_STAGES.findIndex(item => item.key === stage.key) <= GIFT_STAGES.findIndex(item => item.key === currentStage); const recorded = gift.lifecycle?.find(item => item.stage === stage.key); return <button key={stage.key} type="button" disabled={busy} onClick={() => setStage(gift.uuid, stage.key)} title={recorded?.at ? `${stage.label}: ${new Date(recorded.at).toLocaleString('cs-CZ')}` : stage.label} className={`min-h-8 rounded-lg border px-2 text-[10px] disabled:opacity-50 ${done ? 'border-amber-300/35 bg-amber-500/15 text-amber-100' : 'border-[var(--color-border)] text-[var(--color-text-secondary)]'}`}>{done ? '✓ ' : ''}{stage.label}</button>; })}{gift.can_toggle_privacy && <button type="button" disabled={busy} onClick={() => togglePrivacy(gift)} className="inline-flex min-h-8 items-center gap-1 rounded-lg border border-[var(--color-border)] px-2 text-[10px] text-[var(--color-text-secondary)] disabled:opacity-50"><Lock size={12}/>{gift.is_private ? 'Soukromé' : 'Sdílené'}</button>}</div></div>; })}{!gifts.length && <p className="rounded-xl border border-dashed border-[var(--color-border)] p-5 text-center text-sm text-[var(--color-text-secondary)] md:col-span-2">Zatím tu není žádný nápad na dárek.</p>}</div>
    </section>;
}