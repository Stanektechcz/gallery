...
export default function PartnerPulsePanel({ spaceId, initialPulse, compact = false }: { spaceId?: number | null; initialPulse?: PartnerPulse | null; compact?: boolean }) {
    ...
    return (
        <section className="rounded-3xl border border-teal-400/25 bg-gradient-to-br from-teal-500/10 via-[var(--color-bg-card)] to-pink-500/5 p-4 sm:p-5">
            ...
            {pulse.check_in_available && <form onSubmit={saveCheckIn} className="mt-4 grid gap-2 rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface-muted)] p-3 sm:grid-cols-2 lg:grid-cols-5">
                ...
                <div className="flex min-h-10 items-center gap-1 rounded-lg border border-[var(--color-border)] px-2"><span className="mr-1 text-xs text-[var(--color-text-secondary)]">Energie</span>{[1,2,3,4,5].map(value => <button key={value} type="button" aria-label={`Sdílet energii ${value} z 5`} disabled={busy === 'check-in'} onClick={() => shareEnergy(value)} className={`h-7 min-w-7 rounded-md text-xs ${Number(checkIn.energy) === value ? 'bg-teal-500 text-white' : 'bg-[var(--color-surface-muted)] text-teal-100 hover:bg-teal-500/20'}`}>{value}</button>)}</div>
