    ...
    return (
        <section className="rounded-3xl border border-teal-400/25 bg-gradient-to-br from-teal-500/10 via-[var(--color-bg-card)] to-pink-500/5 p-4 sm:p-5">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div><p className="flex items-center gap-2 text-xs font-medium uppercase tracking-wider text-teal-200"><HeartHandshake size={15}/> Partnerský puls</p><h2 className="mt-1 font-semibold text-[var(--color-text-primary)]">Dnešní kapacita a společná zodpovědnost</h2><p className="mt-1 text-xs text-[var(--color-text-secondary)]">Jeden živý přehled nad úkoly, kalendářem, cestami, doklady, dárky, financemi a balením.</p></div>
            ...
            {/* Karty obou lidí vedle sebe i na telefonu. Je v nich jméno, kroky a jedna
                věta o stavu — na sto padesát bodů šířky se to vejde a pod sebou zabíraly
                dvojnásobek výšky na nástěnce, kde je o místo největší nouze. */}
            <div className="mt-4 grid grid-cols-2 gap-2">
                {pulse.members.map(member => <div key={member.id} className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-muted)] p-3"><div className="flex items-center justify-between gap-2"><span className="truncate text-sm font-medium text-[var(--color-text-primary)]">{member.name}</span><span className={`text-xs ${member.overdue_actions ? 'text-amber-200' : 'text-teal-100'}`}>{member.open_actions} kroků</span></div>{member.check_in ? <p className="mt-1 truncate text-xs text-[var(--color-text-secondary)]">{member.check_in.mood ? MOODS[member.check_in.mood] : 'Dnešní stav'} · energie {member.check_in.energy ?? '–'}/5 · {CAPACITY[member.check_in.capacity]}{member.check_in.focus ? ` · ${member.check_in.focus}` : ''}</p> : <p className="mt-1 text-xs text-[var(--color-text-secondary)]">Dnešní stav zatím nesdílel/a.</p>}</div>)}
            </div>
            ...
            {pulse.check_in_available && <form onSubmit={saveCheckIn} className="mt-4 grid grid-cols-2 gap-2 rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface-muted)] p-3 lg:grid-cols-5 [&>div]:col-span-2 [&>input]:col-span-2 [&>button]:col-span-2 lg:[&>div]:col-span-1 lg:[&>input]:col-span-1 lg:[&>button]:col-span-1">
                <select aria-label="Dnešní nálada" value={checkIn.mood} onChange={event => setCheckIn(current => ({...current, mood:event.target.value}))} className="min-h-10 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-card)] px-2 text-xs text-[var(--color-text-primary)]"><option value="">Jak se mám?</option>{Object.entries(MOODS).map(([value,label]) => <option key={value} value={value}>{label}</option>)}</select>
                <div className="flex min-h-10 items-center gap-1 rounded-lg border border-[var(--color-border)] px-2"><span className="mr-1 text-xs text-[var(--color-text-secondary)]">Energie</span>{[1,2,3,4,5].map(value => <button key={value} type="button" aria-label={`Sdílet energii ${value} z 5`} disabled={busy === 'check-in'} onClick={() => shareEnergy(value)} className={`h-9 min-w-9 rounded-md text-xs lg:h-7 lg:min-w-7 ${Number(checkIn.energy) === value ? 'bg-teal-700 text-white' // Neprůhledné pozadí místo pětiprocentního nádechu: na něm vyšel tyrkysový text
                                    // 4,40:1 proti hranici 4,5, na kartě 5,3.
                                    : 'bg-[var(--color-bg-card)] text-teal-100 hover:bg-teal-500/20'}`}>{value}</button>)}</div>
                <select aria-label="Dnešní kapacita" value={checkIn.capacity} onChange={event => setCheckIn(current => ({...current, capacity:event.target.value as CheckIn['capacity']}))} className="min-h-10 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-card)] px-2 text-xs text-[var(--color-text-primary)]">{Object.entries(CAPACITY).map(([value,label]) => <option key={value} value={value}>{label}</option>)}</select>
                <input value={checkIn.focus} onChange={event => setCheckIn(current => ({...current, focus:event.target.value}))} placeholder="Na co se dnes soustředím" className="min-h-10 rounded-lg border border-[var(--color-border)] bg-transparent px-3 text-xs text-[var(--color-text-primary)] placeholder:text-[var(--color-text-secondary)]"/>
                <button disabled={busy === 'check-in'} className="min-h-10 rounded-lg bg-teal-700 px-3 text-xs font-medium text-white disabled:opacity-50">{busy === 'check-in' ? 'Ukládám…' : pulse.my_check_in ? 'Aktualizovat stav' : 'Sdílet dnešní stav'}</button>
                <label className="col-span-2 flex items-center gap-2 text-[11px] text-[var(--color-text-secondary)] lg:col-span-5"><input type="checkbox" checked={checkIn.is_shared} onChange={event => setCheckIn(current => ({...current,is_shared:event.target.checked}))}/> Sdílet s partnerem; jinak zůstane check-in jen mně.</label>
            </form>}
            ...
            {!compact && <div className="mt-4 space-y-2">
                ...
                {pulse.actions.map(action => <div key={action.key} className="grid gap-2 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-muted)] p-3 sm:grid-cols-[auto_minmax(0,1fr)_auto_auto] sm:items-center">
                    ...
                    {action.assignment_locked ? <span className="inline-flex min-h-9 max-w-40 items-center rounded-lg border border-[var(--color-border)] px-2 text-xs text-teal-100">Platí {action.assigned_to?.name}</span> : <select aria-label={`Přiřadit ${action.title}`} value={action.assigned_to?.id ?? ''} onChange={event => mutate(action,{assigned_to:event.target.value ? Number(event.target.value) : null})} className="min-h-9 max-w-40 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-card)] px-2 text-xs text-[var(--color-text-primary)]"><option value="">Domluvit kdo</option>{pulse.members.map(member => <option key={member.id} value={member.id}>{member.name}</option>)}</select>}
                    <button disabled={busy === action.key} onClick={() => mutate(action,{snoozed_until:tomorrow})} className="inline-flex min-h-9 items-center justify-center gap-1 rounded-lg border border-[var(--color-border)] px-2 text-xs text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)]"><AlarmClock size={13}/> zítra</button>
                </div>)}
            </div>}
