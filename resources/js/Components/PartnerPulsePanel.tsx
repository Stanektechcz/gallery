    ...
    return (
        <section className="rounded-3xl border border-teal-400/25 bg-gradient-to-br from-teal-500/10 via-[var(--color-bg-card)] to-pink-500/5 p-4 sm:p-5">
            ...
            {pulse.check_in_available && <form onSubmit={saveCheckIn} className="mt-4 grid grid-cols-2 gap-2 rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface-muted)] p-3 lg:grid-cols-5 [&>div]:col-span-2 [&>input]:col-span-2 [&>button]:col-span-2 lg:[&>div]:col-span-1 lg:[&>input]:col-span-1 lg:[&>button]:col-span-1">
                ...
                <div className="flex min-h-10 items-center gap-1 rounded-lg border border-[var(--color-border)] px-2"><span className="mr-1 text-xs text-[var(--color-text-secondary)]">Energie</span>{[1,2,3,4,5].map(value => <button key={value} type="button" aria-label={`Sdílet energii ${value} z 5`} disabled={busy === 'check-in'} onClick={() => shareEnergy(value)} className={`h-9 min-w-9 rounded-md text-xs lg:h-7 lg:min-w-7 ${Number(checkIn.energy) === value ? 'bg-teal-700 text-white' : 'bg-[var(--color-surface-muted)] text-teal-100 hover:bg-teal-500/20'}`}>{value}</button>)}</div>
