...
export default function ReminderActionPanel({
    ...
    return <section className={`rounded-2xl border border-amber-400/25 bg-gradient-to-br from-amber-500/10 to-[var(--color-bg-card)] ${compact?'p-4':'p-4 sm:p-5'}`}>
        ...
        <div className="mt-4 space-y-2">{items.map(item => {
            ...
            return <article key={item.id} className="rounded-xl border border-amber-300/15 bg-[var(--color-surface-muted)] p-3"><div className="flex items-start gap-3"><span className={`mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${item.status==='failed'?'bg-red-500/15 text-red-200':item.status==='delivered'?'bg-amber-500/20 text-amber-100':'bg-[var(--color-surface-muted)] text-[var(--color-text-secondary)]'}`}><Clock3 size={15}/></span><div className="min-w-0 flex-1">{item.event ? <Link href={`/calendar/events/${item.event.uuid}`} className="block truncate text-sm font-medium text-[var(--color-text-primary)] hover:text-amber-100">{item.event.title}</Link> : <p className="text-sm font-medium text-[var(--color-text-primary)]">Připomínka akce</p>}<p className="mt-1 text-[10px] text-[var(--color-text-secondary)]">{statusLabel[item.status]??item.status} · {dateTime(item.remind_at)}{item.recipient_name?` · ${item.recipient_name}`:''}{item.snooze_count?` · odloženo ${item.snooze_count}×`:''}</p>{item.event?.place_name&&<p className="mt-1 truncate text-[10px] text-[var(--color-text-secondary)]">📍 {item.event.place_name}</p>}{item.last_error&&<p className="mt-1 text-[10px] text-red-200">{item.last_error}</p>}</div></div>{actionable&&<div className="mt-3 flex flex-wrap gap-1.5 border-t border-amber-300/10 pt-3"><button type="button" disabled={!!busy} onClick={()=>act(item,'snooze',{minutes:60})} className="min-h-9 rounded-lg border border-[var(--color-border)] px-2.5 text-[10px] text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] disabled:opacity-40">Za 1 hodinu</button>{canTomorrow&&<button type="button" disabled={!!busy} onClick={()=>act(item,'snooze',{minutes:1440})} className="min-h-9 rounded-lg border border-[var(--color-border)] px-2.5 text-[10px] text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] disabled:opacity-40">Zítra</button>}<button type="button" disabled={!!busy} onClick={()=>act(item,'acknowledge')} className="inline-flex min-h-9 items-center gap-1 rounded-lg bg-emerald-500/15 px-2.5 text-[10px] text-emerald-100 disabled:opacity-40"><Check size={12}/> Vyřízeno</button><button type="button" disabled={!!busy} onClick={()=>act(item,'dismiss')} className="ml-auto inline-flex min-h-9 items-center gap-1 rounded-lg px-2 text-[10px] text-[var(--color-text-secondary)] hover:bg-[var(--color-surface-hover)] hover:text-[var(--color-text-primary)] disabled:opacity-40"><X size={12}/> Skrýt</button></div>}{!compact&&item.history&&item.history.length>0&&<details className="mt-2 border-t border-amber-300/10 pt-2"><summary className="cursor-pointer text-[10px] text-[var(--color-text-secondary)]">Historie připomínky ({item.history.length})</summary><div className="mt-2 space-y-1">{item.history.slice(0,5).map((entry,index)=><p key={`${entry.status}-${entry.created_at}-${index}`} className="text-[10px] text-[var(--color-text-secondary)]"><span className="text-[var(--color-text-primary)]/80">{historyLabel[entry.status]??entry.status}</span> · {dateTime(entry.created_at)}{entry.detail?` · ${entry.detail}`:''}</p>)}</div></details>}</article>;
