...
export default function CalendarEventEditor({ eventUuid, open, onClose, onSaved, onDeleted }: {
    ...
    const inputClass = 'mt-1 min-h-11 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-muted)] px-3 py-2 text-sm text-white outline-none focus:border-[var(--color-accent)]';
    const sectionClass = 'rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface-muted)] p-4';
        ...
        <div className="fixed inset-0 z-[850] flex items-end bg-black/70 sm:items-center sm:justify-center sm:p-4" role="dialog" aria-modal="true" aria-label="Správa kalendářní akce">
            ...
            <form onSubmit={submit} className="safe-area-pb relative z-10 flex max-h-[96dvh] w-full max-w-4xl flex-col overflow-hidden rounded-t-3xl border border-[var(--color-border)] bg-[var(--color-bg-secondary)] shadow-2xl sm:max-h-[92dvh] sm:rounded-3xl">
                <header className="flex shrink-0 items-center gap-3 border-b border-[var(--color-border)] px-4 py-3 sm:px-5">
                    ...
                    <div className="min-w-0 flex-1"><h2 className="truncate font-semibold text-[var(--color-text-primary)]">Správa kalendářní akce</h2><p className="text-xs text-[var(--color-text-secondary)]">Změny se propíšou do cesty, připomínek a společných přehledů.</p></div>
                    <button type="button" onClick={onClose} aria-label="Zavřít" className="flex h-11 w-11 items-center justify-center rounded-xl text-[var(--color-text-secondary)] hover:bg-[var(--color-surface-hover)] hover:text-[var(--color-text-primary)]"><X size={20}/></button>
                </header>
                ...
                <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-4 sm:p-5">
                    ...
                    {loading ? <p className="py-16 text-center text-sm text-[var(--color-text-secondary)]">Načítám všechny vazby akce…</p> : event && !event.can_edit ? <p className="py-16 text-center text-sm text-amber-200">Tuto akci můžete zobrazit, ale nemáte oprávnění ji měnit.</p> : event ? <div className="grid gap-4 lg:grid-cols-2">
                        ...
                        <section className={`${sectionClass} lg:col-span-2`}>
                            <h3 className="font-medium text-[var(--color-text-primary)]">Základní informace</h3>
                        ...
                        <section className={sectionClass}>
                            <h3 className="flex items-center gap-2 font-medium text-[var(--color-text-primary)]"><CalendarClock size={16} className="text-[var(--color-accent)]"/>Termín a opakování</h3>
                            <label className="mt-3 flex min-h-11 items-center gap-2 rounded-xl border border-[var(--color-border)] px-3 text-sm text-[var(--color-text-primary)]"><input type="checkbox" checked={form.all_day} onChange={change => setForm({...form, all_day:change.target.checked})}/>Celodenní akce</label>
                        ...
                        <section className={sectionClass}>
                            <h3 className="flex items-center gap-2 font-medium text-[var(--color-text-primary)]"><MapPin size={16} className="text-[var(--color-accent)]"/>Místo a odjezd</h3>
                        ...
                        <section className={sectionClass}>
                            <h3 className="flex items-center gap-2 font-medium text-[var(--color-text-primary)]"><Users size={16} className="text-pink-300"/>Účastníci a soukromí</h3>
                        ...
                        <section className={sectionClass}>
                            <div className="flex items-start justify-between gap-3"><div><h3 className="flex items-center gap-2 font-medium text-[var(--color-text-primary)]"><Bell size={16} className="text-amber-300"/>Připomínky</h3><p className="mt-1 text-[10px] text-[var(--color-text-secondary)]">Připomínky vytvořené jinými částmi systému zůstávají zachované.</p></div><button type="button" onClick={addReminder} disabled={reminders.length >= 12 || form.status === 'cancelled'} className="inline-flex min-h-10 items-center gap-1 rounded-xl border border-amber-300/25 px-3 text-xs text-amber-100 disabled:opacity-40"><Plus size={14}/>Přidat</button></div>
                        ...
                        <section className={`${sectionClass} lg:col-span-2`}>
                            <h3 className="flex items-center gap-2 font-medium text-[var(--color-text-primary)]"><Route size={16} className="text-teal-300"/>Propojení s cestováním</h3>
