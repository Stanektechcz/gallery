    ...
    const renderCmd = (cmd: Command, idx: number) => {
        ...
        return (
            <button key={`${cmd.group}-${cmd.label}`}
                ...
                className={`w-full text-left px-4 py-2 text-sm flex items-center gap-3 transition-colors ${isActive ? 'bg-[var(--color-accent)]/15 text-[var(--color-accent-contrast)]' : 'text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)]'}`}>
    ...
    return (
        <div className="relative">
            <button type="button" aria-label="Otevřít partnerská upozornění" aria-expanded={open} onClick={() => setOpen(v => !v)}
                ...
                <Bell size={18}/>
                {unread > 0 && (
                    <span className="absolute top-0.5 right-0.5 w-4 h-4 bg-[var(--color-accent)] text-[var(--color-accent-contrast)] text-[9px] font-bold rounded-full flex items-center justify-center">
                        {unread > 9 ? '9+' : unread}
                    </span>
                )}
            </button>
...
export const navGroups: NavigationGroup[] = [
    {
        ...
        items: [
            ...
            { href: '/cyklus', label: 'Cyklus', icon: Droplet },
            { href: '/duplicity', label: 'Duplicity', icon: Copy },
        ...
        <div className="fixed inset-0 z-[850] flex items-end justify-center p-0 sm:items-center sm:p-4" role="dialog" aria-modal="true" aria-label="Přizpůsobit hlavní nabídku">
            ...
            <section className="safe-area-pb relative z-10 flex max-h-[88dvh] w-full max-w-xl flex-col overflow-hidden rounded-t-3xl border border-[var(--color-border)] bg-[var(--color-bg-secondary)] shadow-2xl sm:rounded-3xl">
                                            ...
                                            return (
                                                <div key={item.href} className="flex min-h-12 items-center gap-2 rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-card)] px-2">
                                                    <button type="button" onClick={() => toggle(item.href)} disabled={!checked && pinnedHrefs.length >= 6} className="flex min-h-11 min-w-0 flex-1 items-center gap-3 px-1 text-left disabled:opacity-40" aria-pressed={checked}>
                                                        ...
                                                        <span className={clsx('h-5 w-5 shrink-0 rounded-md border text-center text-xs leading-[18px]', checked ? 'border-[var(--color-accent)] bg-[var(--color-accent)] text-[var(--color-accent-contrast)]' : 'border-[var(--color-border)] text-transparent')}>✓</span>
                                                    </button>
                                                    {checked && (
                ...
                <footer className="flex gap-2 border-t border-[var(--color-border)] p-3 sm:p-4">
                    ...
                    <button type="button" onClick={onClose} className="min-h-11 flex-1 rounded-xl bg-[var(--color-accent)] px-4 text-sm font-medium text-[var(--color-accent-contrast)]">Hotovo</button>
                </footer>
            </section>
        </div>
    );
