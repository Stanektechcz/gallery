...
class UploadManagerClass extends EventTarget {
    ...
    constructor() {
        ...
        window.addEventListener('offline', () => { this.online = false; this.handleOffline(); });
    }
    ...
    enqueue(files: File[], albumId: number | null = null): void {
        for (const file of files) {
            ...
            const item: ManagedUpload = {
                id, file, filename: file.name, size: file.size, albumId,
                status: 'waiting', percent: 0, thumb,
    ...
    pauseAll(): void {
        ...
        this.queue.filter(u => u.status === 'waiting').forEach(u => {
            u.status = 'paused'; this.pausedIds.add(u.id);
        });
        this.emit();
    }
    ...
    resumeAll(): void {
        ...
        this.queue.filter(u => u.status === 'paused').forEach(u => { u.status = 'waiting'; });
        this.emit();
        this.process();
    }

    clearDone(): void {
        const removed = this.queue.filter(u => ['done', 'duplicate', 'cancelled'].includes(u.status));
        removed.forEach(u => { if (u.thumb) URL.revokeObjectURL(u.thumb); });
        this.queue = this.queue.filter(u => !['done', 'duplicate', 'cancelled'].includes(u.status));
        this.emit();
    }

    getAll(): ManagedUpload[] { return [...this.queue]; }
    ...
    getStats() {
        ...
        return {
            ...
            uploading: all.filter(u => ['uploading','hashing'].includes(u.status)).length,
            done:      all.filter(u => ['done','duplicate'].includes(u.status)).length,
    ...
    private process(): void {
        ...
        while (this.activeCount() < MAX_CONCURRENT) {
            const next = this.queue.find(u => u.status === 'waiting' && !this.pausedIds.has(u.id));
            if (!next) break;
    ...
    private async runUpload(item: ManagedUpload): Promise<void> {
        ...
        try {
            ...
            if (item.sha256) {
                const dupRes = await axios.post('/api/v1/uploads/check-duplicate', { sha256: item.sha256 }, { signal: ctrl.signal });
            ...
            if (!sessionUuid) {
                const init = await axios.post('/api/v1/uploads', {
                ...
                const statusRes = await axios.get(`/api/v1/uploads/${sessionUuid}`, { signal: ctrl.signal });
            ...
            for (let idx = 0; idx < item.totalChunks; idx++) {
                ...
                await axios.put(`/api/v1/uploads/${sessionUuid}/chunks/${idx}`, form, {
                ...
                item.percent = Math.round(((item.uploadedChunks.length) / item.totalChunks) * 90);
                this.emit();
            }
            ...
            const completeRes = await axios.post(`/api/v1/uploads/${sessionUuid}/complete`, {}, { signal: ctrl.signal });
            item.mediaUuid = completeRes.data.media_uuid ?? completeRes.data.uuid;
            item.status  = 'done';
            item.percent = 100;
            ...
            if (ctrl.signal.aborted || err?.code === 'ERR_CANCELED') {
                ...
                if (item.status === 'uploading' || item.status === 'hashing') {
                    item.status = this.pausedIds.has(item.id) ? 'paused' : 'cancelled';
                }
    ...
    private handleOnline(): void {
        ...
        this.queue.filter(u => u.status === 'offline').forEach(u => { u.status = 'waiting'; });
        this.emit();
        this.process();
    }

    private handleOffline(): void {
        this.queue.filter(u => u.status === 'uploading').forEach(u => {
            this.aborts.get(u.id)?.abort();
            this.aborts.delete(u.id);
            u.status = 'offline';
        });
    ...
    private overallPercent(): number {
        const relevant = this.queue.filter(u => !['cancelled'].includes(u.status));
        if (relevant.length === 0) return 0;
        const total = relevant.reduce((sum, u) => sum + (u.status === 'done' || u.status === 'duplicate' ? 100 : u.percent), 0);
        return Math.round(total / relevant.length);
    }

    private emit(): void {
        this.dispatchEvent(new CustomEvent('change', { detail: { uploads: [...this.queue] } }));
    }
